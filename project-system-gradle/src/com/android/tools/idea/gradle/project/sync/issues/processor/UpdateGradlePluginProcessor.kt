...
class UpdateGradlePluginProcessor(val project: Project, private val pluginToVersionMap: Map<GradlePluginInfo, String>) :
  ...
  override fun findUsages(): Array<UsageInfo> {
    return getIncludedBuilds(project).flatMap { getUsages(it) }.toTypedArray()
  }

  override fun performRefactoring(usages: Array<out UsageInfo>) {
    val gradlePluginUsages =
      usages.filterIsInstance<GradlePluginUsageInfo>().filter {
    ...
    gradlePluginUsages.map { it.projectBuildModel }.distinct().forEach { it.applyChanges() }
  }
  ...
  private fun getUsages(projectBuildModel: ProjectBuildModel): List<UsageInfo> =
    projectBuildModel.allIncludedBuildModels.flatMap { getUsages(projectBuildModel, it) }
  ...
  private fun getUsages(projectBuildModel: ProjectBuildModel, gradleBuildModel: GradleBuildModel): List<UsageInfo> {
    ...
    return artifactDependencies.mapNotNull {
      val newVersion = pluginToVersionMap[GradlePluginInfo(it.name().toString(), it.group().toString())] ?: return@mapNotNull null
...
/**
 * This method returns a list of all participating [ProjectBuildModel] from a given [Project]. This includes any included builds. No
 * ordering is guaranteed.
 *
 * This method should never be called on the UI thread, it will cause the parsing of Gradle build files which can take a long time. The
 * returned [ProjectBuildModel] is not thread safe.
 *
...
fun getIncludedBuilds(project: Project): List<ProjectBuildModel> {
  ...
  result.addAll(
    compositeBuild.compositeParticipants
