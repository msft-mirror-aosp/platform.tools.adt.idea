 ...
 * from Gradle during sync and if this exception is recognised during exception handling process the corresponding value will be collected
 * in this service. In the end, in `onSyncFailure` listener call, collected value is reported to the metrics. This service waits for
 * [FinishBuildEvent] to be sent to [SyncViewManager], it subscribes to the [SyncViewManager] at sync start for this. Waiting for this event
 * guarantees that both IssueChecker error handling flow and [BuildOutputParser] flow finished processing, so we have all error-related data
 * to report.
 */
...
class SyncFailureUsageReporter {
  ...
  fun onSyncStart(externalSystemTaskId: ExternalSystemTaskId?, project: Project, rootProjectPath: @SystemIndependent String) {
    ...
    Disposer.register(disposable) { collectedFailureDetailsByBuildId.remove(externalSystemTaskId) }
