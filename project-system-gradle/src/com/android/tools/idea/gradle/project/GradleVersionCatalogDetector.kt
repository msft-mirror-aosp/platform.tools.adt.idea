...
/**
 * This class and its methods attempt to answer the question of whether a given (Gradle-based) project uses the Version Catalog feature,
 * introduced as experimental in Gradle version 7.0 and made stable in version 7.4. (Once Studio support for Version Catalogs is stable and
 * documented, this class can probably go away.
 *
 * A project uses version catalogs if:
 * 1. its Gradle version is greater than 7.4, and a. it has a versionCatalogs clause in its settings file; or if not b. it has a
 *    gradle/libs.versions.toml file relative to its base directory
 * 2. its Gradle version is between 7.0 and 7.4, and it has an enableFeaturePreview("VERSION_CATALOGS") call in its settings file, and a. it
 *    has a versionCatalogs clause in its settings file; or if not b. it has a gradle/libs.versions.toml file relative to its base directory
 */
class GradleVersionCatalogDetector(private val project: Project) : Disposable {
      ...
      object : DocumentListener {
        override fun documentChanged(event: DocumentEvent) {
          ...
          when (file.name) {
            ...
            FN_SETTINGS_GRADLE,
            FN_SETTINGS_GRADLE_KTS -> _settingsVisitorResults = null
          }
        }
      }
  ...
  companion object {
    @JvmStatic
    fun getInstance(project: Project): GradleVersionCatalogDetector = runReadAction {
  ...
  private val gradleVersion: GradleVersion
    get() {
      ...
      val gradleVersion =
        gradleWrapper?.gradleVersion?.let { runCatching { GradleVersion.version(it) }.getOrNull() } ?: MISSING_GRADLE_VERSION
      return gradleVersion.also { _gradleVersion = gradleVersion }
    }

  private val settingsVisitorResults: SettingsVisitorResults
    get() {
      _settingsVisitorResults?.let {
        return it
      }
      return runReadAction {
        ...
        val settingsVisitorResults =
          when (val settingsPsiFile = PsiManager.getInstance(project).findFile(settingsFile)) {
  ...
  class VersionCatalogTomlSuggestion :
    Notification("Gradle Version Catalog DSL", AndroidBundle.message("project.gradle.catalog.settings.dsl"), NotificationType.INFORMATION) {
    init {
  ...
  private fun visitGroovySettings(settingsPsiFile: GroovyFile): SettingsVisitorResults {
    val visitor =
      object : GroovyRecursiveElementVisitor(), SettingsVisitorResults {
  ...
  private fun visitKtSettings(settingsPsiFile: KtFile): SettingsVisitorResults {
    val visitor =
      object : KtTreeVisitorVoid(), SettingsVisitorResults {
        override var enableFeaturePreview = false
        override var versionCatalogsCall = false
        override var catalogEntry = false
