...
private fun findFacetsToMigrate(project: Project): List<AndroidFacet> {
  return ProjectFacetManager.getInstance(project).getFacets(AndroidFacet.ID).filter { facet ->
    ...
    val shouldMigrateDefaultTargetSdk =
      projectProperties?.findPropertyByKey(DEFAULT_TARGET_SDK_TO_COMPILE_SDK_IF_UNSET_PROPERTY)?.value == "false"
...
class MigrateToDefaultTargetSdkToCompileSdkIfUnsetAction : AndroidGradleBaseRefactoringAction() {
  ...
  override fun isAvailableOnElementInEditorAndFile(element: PsiElement, editor: Editor, file: PsiFile, context: DataContext) =
    isEnabledOnProject(element.project)
...
class MigrateToDefaultTargetSdkToCompileSdkIfUnsetHandler : RefactoringActionHandler {
  ...
  private fun invoke(project: Project) {
    ...
    // If for some reason Android Gradle Plugin version cannot be found, assume users have the correct version for the refactoring i.e.
    // 8.11.0
...
class MigrateToDefaultTargetSdkToCompileSdkIfUnsetProcessor
private constructor(
  project: Project,
  ...
  override fun findUsages(): Array<UsageInfo> {
    ...
    if (updateTopLevelGradleProperties) {
      ...
      if (propertiesFile != null) {
        allUsageInfos.add(PropertiesUsageInfo(DEFAULT_TARGET_SDK_TO_COMPILE_SDK_IF_UNSET_PROPERTY, propertiesFile.containingFile))
      }
    }
  ...
  override fun performRefactoring(usages: Array<out UsageInfo>) {
    ...
    if (updateTopLevelGradleProperties) {
      ...
      if (propertiesFile != null) {
        when {
          onByDefault -> {
            propertiesFile.findPropertyByKey(DEFAULT_TARGET_SDK_TO_COMPILE_SDK_IF_UNSET_PROPERTY)?.psiElement?.delete()
          }

          available -> {
            propertiesFile.findPropertyByKey(DEFAULT_TARGET_SDK_TO_COMPILE_SDK_IF_UNSET_PROPERTY)?.setValue("true")
          }
  ...
  override fun performPsiSpoilingRefactoring() {
    ...
    val gradlePropertiesFile =
      myProject.getProjectProperties(false)?.virtualFile?.let { PsiManager.getInstance(myProject).findFile(it) as? PropertiesFileImpl }
  ...
  class MigrateUsageInfo(element: PsiElement, val buildModel: ProjectBuildModel, val module: Module, val minSdk: AndroidVersion) :
