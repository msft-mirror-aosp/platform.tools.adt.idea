...
class JavaLanguageLevelDeprecationOutputParserTest : BuildOutputIntegrationTestBase() {
   ...
   * -SetJavaLanguageLevelAllQuickFix -PickLanguageLevelInPSDQuickFix -Open<Source|Target>CompatibilityLinkQuickFix
  ...
  fun testJava7OnJDK21WithJavaLib() {
    ...
    preparedProject.root.resolve("lib").let { lib ->
      ...
      lib
        .resolve("src/main/java/org/example/LibClass.java")
        .writeText(
          """
          package org.example;
    ...
    verifyStats(JAVA_NOT_SUPPORTED_LANGUAGE_LEVEL, JAVA_NOT_SUPPORTED_LANGUAGE_LEVEL)
  }
  ...
  private fun PreparedTestProject.getBuildIssues(
    javaSdkVersion: JavaSdkVersion,
    languageLevel: LanguageLevel,
    expectSuccess: Boolean,
  ): List<BuildEvent> {
    return open(updateOptions = { it.copy(overrideProjectGradleJdkPath = getEmbeddedJdkPathWithVersion(javaSdkVersion)) }) { project ->
