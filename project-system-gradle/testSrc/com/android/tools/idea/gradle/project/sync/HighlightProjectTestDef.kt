...
data class HighlightProjectTestDef(
  ...
  override fun PreparedTestProject.Context.runTest(root: File, project: Project, expect: Expect) {
    modulesAndFiles.forEach { (gradleProjectPath, filePaths) ->
      val module =
        (gradleProjectPath.copy(buildRoot = PathUtil.toSystemIndependentName(root.resolve(root).path)).resolveIn(project)
      ...
      filePaths
        .map { File(moduleRoot).resolve(it).path }
        .forEach { path ->
  ...
  companion object {
    val tests: List<HighlightProjectTestDef> =
      listOf(
        HighlightProjectTestDef(
          TestProject.SIMPLE_APPLICATION,
          modulesAndFiles =
            mapOf(
              GradleSourceSetProjectPath("/", ":app", IdeModuleWellKnownSourceSet.MAIN) to
                listOf("src/main/java/google/simpleapplication/MyActivity.java")
            ),
        ),
        HighlightProjectTestDef(
        ...
        HighlightProjectTestDef(
          ...
          modulesAndFiles =
            mapOf(
              GradleSourceSetProjectPath("/", ":app", IdeModuleWellKnownSourceSet.ANDROID_TEST) to
                listOf("src/androidTest/java/com/example/app/ExampleInstrumentedTest.kt")
            ),
        ),
        HighlightProjectTestDef(
        ...
        HighlightProjectTestDef(
          ...
          modulesAndFiles =
            mapOf(
              GradleSourceSetProjectPath("/", ":kmpFirstLib", wellKnownOrCreate("androidDeviceTest")) to
    ...
    private fun validateRClassResolutionInKmpAndroid(fixture: JavaCodeInsightTestFixture) {
      ...
      assertThat(fixture.lookupElementStrings).containsAllOf("drawable", "layout")
      ...
      assertThat((elementAtCaret.parent.parent as TransitiveAarRClass).packageName).isEqualTo("androidx.appcompat")
    }
  }
}
