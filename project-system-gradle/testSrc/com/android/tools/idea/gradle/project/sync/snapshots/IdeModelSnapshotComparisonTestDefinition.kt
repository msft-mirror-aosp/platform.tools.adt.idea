 ...
 *
 * These tests convert Ide models to a stable text format which does not depend on local environment (and ideally should not depend on the
 ...
 * "//tools/adt/idea/android:intellij.android.core.tests_tests__all --test_filter=IdeV2ModelSnapshotComparisonTest".
 */
data class IdeModelSnapshotComparisonTestDefinition(
  ...
  companion object {
    fun tests(): List<IdeModelSnapshotComparisonTestDefinition> =
      listOf(
        ...
        IdeModelSnapshotComparisonTestDefinition(TestProject.SIMPLE_APPLICATION_NOT_AT_ROOT, skipV1toV2Comparison = true),
        IdeModelSnapshotComparisonTestDefinition(TestProject.SIMPLE_APPLICATION_MULTIPLE_ROOTS, skipV1toV2Comparison = true),
        IdeModelSnapshotComparisonTestDefinition(TestProject.SIMPLE_APPLICATION_WITH_UNNAMED_DIMENSION, skipV1toV2Comparison = true),
...
/**
 * we skip: [com.android.tools.idea.gradle.model.IdeAndroidLibrary.lintJar] because in V2 we do check that the jar exists before populating
 * the property. [com.android.tools.idea.gradle.model.IdeVariant.deprecatedPreMergedApplicationId] as not present in V2
 ...
 * [com.android.tools.idea.gradle.model.IdeBaseArtifact.runtimeClasspath] as it is not available in V1. LIBRARY_TABLE/library as the V2
 * model contains extra libraries not present in the V1 version
 */
private val PROPERTIES_TO_SKIP =
  setOf(
...
private val ENTITIES_TO_SKIP =
  setOf("/Dependencies/compileClasspath", "/Dependencies/runtimeClasspath", "/MainArtifact/ModelSyncFile", "LIBRARY_TABLE/library")

/**
 * some properties values have different patterns in V1 and V2, and we do suppress them. AndroidLibrary: in V2 we added better support for
 * wrapped_aars and composite builds in the libraries names, so this will end up being different from V1 where we do not have support for
 * composite builds, and the names are prefixed with "artifacts" instead. AndroidLibrary.ArtifactAddress: the same rules from above apply to
 * ArtifactAddress as well, plus the distinction of local aars paths.
 */
private val VALUES_TO_SUPPRESS =
  mapOf(
