...
fun <T : Any> Project.dumpAndroidProjectView(
  ...
  fun PresentationData.toTestText(): String {
    ...
    fun Icon.toText(): String? =
      when {
        // After commit 2c486969f4 evaluate will throw an assertion failed exception if run under
        ...
        this is LayeredIcon -> {
          ...
          // When running from bazel some of NDK files are symlinked, while running from IDE direct
          // path is used
  ...
  fun createAndDumpProjectView(): String {
    ...
    // In production sorting happens when the tree builder asynchronously populates the UI. It uses
    // the following comparator, by default,
  ...
  fun getCurrentSettings(): ProjectViewSettings =
    ProjectView.getInstance(this).let { view ->
      ProjectViewSettings(
