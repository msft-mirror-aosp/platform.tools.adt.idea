...
/**
 * Represents a dependency on an external library. External libraries are folders containing some combination of prebuilt classes,
 * resources, and manifest file. Although android libraries are normally packaged in an AAR file, the actual dependency is on the folder
 * where the AAR would have been extracted by the build system rather than the AAR file itself. If the library came from an AAR file, the
 * build system would extract it somewhere and then provide an instance of [ExternalAndroidLibrary] describing the contents and location of
...
interface ExternalAndroidLibrary {
   ...
   *
   * The IDE doesn't work with AAR files and instead relies on the build system to extract necessary files to disk. Location of the original
   * AAR files is not always known, and some [ExternalAndroidLibrary] instances point to folders that never came from an AAR. In such cases,
   * this attribute is null.
   */
  val location: PathString?

  /**
   * Location of the manifest file for this library. This manifest contains sufficient information to understand the library and its
   * contents are intended to be merged into any application that uses the library.
   *
   * Any library that contains resources must either provide a [manifestFile] or a [packageName]. Not all libraries include a manifest. For
   * example, some libraries may not contain resources. Other libraries may contain resources but use some other mechanism to inform the
   * build system of the package name. The latter will fill in [packageName] rather than providing a [manifestFile].
   */
  ...
  /** Path to the folder containing unzipped, plain-text, non-namespaced resources. Or null for libraries that contain no resources. */
  ...
  /**
   * Path to the symbol file (`R.txt`) containing information necessary to generate the non-namespaced R class for this library. Null if no
   * such file exists.
   */
  ...
  /**
   * Returns true if this [ExternalAndroidLibrary] contributes any resources. Resources may be packaged as either a res.apk file or a res
   * folder.
   */
...
data class ExternalLibraryImpl(
  ...
  /**
   * Constructs a new [ExternalAndroidLibrary] with the given address and all other values set to their defaults. Intended to simplify
   ...
   * folder.
   */
  override val hasResources
    get() = resApkFile != null || resFolder != null
