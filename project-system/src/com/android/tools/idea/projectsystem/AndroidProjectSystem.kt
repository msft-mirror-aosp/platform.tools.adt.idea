...
/**
 * Provides a build-system-agnostic interface to the build system. Instances of this interface only apply to a specific [Project].
 * Instantiating a generalized instance of this interface must, with when considering the wider system, be idempotent with respect to
 * instantiating other instances for the same project: any state needed by the project system must be encapsulated for example in project
 * services.
 */
interface AndroidProjectSystem : ModuleHierarchyProvider {
   ...
   *
   * NOTE: The returned application id provider represents the current build configuration and may become invalid if it changes, hence this
   * reference should not be cached.
   *
   * Some project systems may be unable to retrieve the package name if no [runConfiguration] is provided or before the project has been
   * successfully built. The returned [ApplicationIdProvider] will throw [ApkProvisionException]'s or return a name derived from incomplete
   ...
   *
   * NOTE: The returned apk provider represents the current build configuration and may become invalid if it changes, hence this reference
  ...
  /**
   * [SourceProvidersFactory] instance used by the project system internally to re-instantiate the cached instance when the structure of the
   * project changes.
   */
  ...
  /**
   * Returns a collection of [AndroidFacet]s corresponding to project system entities. Note that this can be different from the collection
   * of all [AndroidFacet]s in the project, if (for example) the project system represents a project system entity with more than one
   * module, each with their own [AndroidFacet].
   */
   ...
   *
   * For example, if the project contains `com.example.myapp` and `com.example.mylib`, this would return true for exactly `com`,
   * `com.example`, `com.example.myapp` and `com.example.mylib`.
   *
   * This method may return false for packages that do exist if it is called when project sync has failed, or for non-gradle build systems
   * if it is called before indexes are ready, but it should never throw IndexNotReadyException.
   */
   ...
   *
   * The application ID might be from a different variant than the currently selected variant, e.g. if an app is set up to have release
   * variant application id com.example.myapp, but the debug variant application id is com.example.myapp.debug, this method will return the
   * app main module for both of those application IDs, irrespective of which is the currently active variant in the IDE.
   *
   * @return Candidate modules to use for attachment configuration. The collection may be empty if no suitable module exists.
   */
  ...
  /**
   * Return a [Comparator] for (partially) ordering modules in a project-system-specific way, suitable for use by [minWith] to find the most
   * production-relevant module(s). (This is inherently a vague specification; callers should not be critically dependent on the order
...
/**
 * Returns a list of all Android holder modules. For Gradle projects, these are the intellij [Module] objects that correspond to an emptyish
 * (no roots/deps) module that contains the other source set modules as children. If you need to obtain the module for the currently active
 * production source set, use [AndroidModuleSystem.getProductionAndroidModule] on the returned [Module] objects.
 */
 ...
 * method; for example, representing individual SourceSets as IDEA [Module]s each with an [AndroidFacet] attached. Facets corresponding to
 * these subsidiary modules are filtered out here by the Project System.
 */
...
fun isScreenshotTestFile(project: Project, file: VirtualFile?): Boolean =
  ReadAction.nonBlocking<Boolean> {
      val module = file?.let { ProjectFileIndex.getInstance(project).getModuleForFile(file) }
