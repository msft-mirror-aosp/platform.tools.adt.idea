...
data class Filters(
  ...
  val operatingSystems: MultiSelection<WithCount<OperatingSystemInfo>> = MultiSelection.emptySelection(),
  ...
  fun withVersions(value: Set<Version>) = copy(versions = versions.selectMatching { it.value in value })
  ...
  fun withFatalityToggle(vararg toggles: FailureType) =
    copy(failureTypeToggles = toggles.fold(failureTypeToggles) { acc, fatality -> acc.toggle(fatality) })

  fun withDevices(value: Set<Device>) = copy(devices = devices.selectMatching { it.value in value })

  fun withOperatingSystems(value: Set<OperatingSystemInfo>) = copy(operatingSystems = operatingSystems.selectMatching { it.value in value })

  fun withSignal(value: SignalType) = copy(signal = signal.select(value))

  fun withVisibilityType(value: VisibilityType) = copy(visibilityType = visibilityType.select(value))
}
...
data class AppInsightsState(
  ...
  val selectedVariant: IssueVariant?
    get() = if (currentIssueVariants is LoadingState.Ready) currentIssueVariants.value?.selected else null
...
fun AppInsightsState.toIssueRequest(clock: Clock): IssueRequest? {
  ...
  return IssueRequest(
    ...
    filters =
      QueryFilters(
        interval = clock.instant().let { Interval(startTime = it.minus(Duration.ofDays(selectedTimeInterval.numDays)), endTime = it) },
        versions =
          if (filters.versions.allSelected()) setOf(Version.ALL) else filters.versions.selected.asSequence().map { it.value }.toSet(),
        devices = if (filters.devices.allSelected()) setOf(Device.ALL) else filters.devices.selected.asSequence().map { it.value }.toSet(),
        operatingSystems =
