...
class Screenshot(filename: String, bitmapType: BitmapType) {
  ...
  init {
    ...
    if (bitmapType == BitmapType.RGB_565) {
      val dataElements = image.raster.getDataElements(0, 0, image.width, image.height, ShortArray(image.width * image.height)) as ShortArray
      dataElements.flatMapTo(imageBytes) { listOf((it.toInt() and 0xFF).toByte(), (it.toInt() ushr 8).toByte()) }
    } else {
      val dataElements = image.raster.getDataElements(0, 0, image.width, image.height, IntArray(image.width * image.height)) as IntArray
