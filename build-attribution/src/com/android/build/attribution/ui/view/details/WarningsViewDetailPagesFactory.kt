...
class WarningsViewDetailPagesFactory(
  ...
  fun createDetailsPage(pageId: WarningsPageId): JComponent =
    if (pageId == WarningsPageId.emptySelection) {
      JPanel().apply {
        layout = BorderLayout()
        name = "empty-details"
        val messageLabel =
      ...
      //  It defines the page, not the tree node. (There can be several nodes for the same page, e.g. task has 2 warnings and shown under
      // both types)
  ...
  @VisibleForTesting
  fun createDetailsPage(nodeDescriptor: WarningsTreePresentableNodeDescriptor): JComponent =
    when (nodeDescriptor) {
      ...
      is PluginGroupingWarningNodeDescriptor ->
        createPluginDetailsPage(nodeDescriptor.pluginName, nodeDescriptor.presentedTasksWithWarnings)
  ...
  private fun createPluginDetailsPage(pluginName: String, tasksWithWarnings: Map<TaskUiData, List<TaskIssueUiData>>) =
    JPanel().apply {
      layout = BorderLayout()
  ...
  private fun createTaskWarningTypeDetailsPage(warningType: TaskIssueType, warnings: List<TaskIssueUiData>) =
    JPanel().apply {
      layout = BorderLayout()
  ...
  private fun createTaskCategoryWarningDetailsPage(taskCategoryData: CriticalPathTaskCategoryUiData) =
    JPanel().apply {
      layout = BorderLayout()
  ...
  private fun createAnnotationProcessorsRootDetailsPage(annotationProcessorsReport: AnnotationProcessorsReport) =
    JPanel().apply {
      layout = BorderLayout()
  ...
  private fun createAnnotationProcessorDetailsPage(annotationProcessorData: AnnotationProcessorUiData) =
    JPanel().apply {
      ...
      val linkPanel =
        JPanel().apply {
          ...
          add(
            HyperlinkLabel("Make sure that you are using an incremental version of this annotation processor.").apply {
