...
private val renderLayoutlibNativeMemoryBenchmark =
  Benchmark.Builder("DesignTools Layoutlib Native Memory Usage Benchmark")
    .setDescription("Base line for RenderTask layoutlib native memory usage (mean) after $NUMBER_OF_SAMPLES samples.")
    .build()

/**
 * A measurement for the given [metric]. This class will be called before and after a profiled operation is executed. [after] is expected to
 * return the [MetricSample] of one execution.
 */
...
private class DelegateMetricMeasurementWithAnalyzers<T>(private val delegate: MetricMeasurement<T>, override val analyzers: Set<Analyzer>) :
...
internal class ElapsedTimeMeasurement<T>(metric: Metric) : MetricMeasurementAdapter<T>(metric) {
  ...
  override fun after(result: T) = MetricSample(Instant.now().toEpochMilli(), System.currentTimeMillis() - startMs)
}

/**
 * A [MetricMeasurement] that measures the JVM heap memory usage using the [HeapSnapshotTraverseService].
 *
 * When [component] is null, the measure is done over the whole [category]. Otherwhise, the measure is done only over the given [component],
 * which must be part of th given [category]. This does not track native memory allocated through JNI.
 *
 ...
 */
internal class HeapSnapshotMemoryUseMeasurement<T>(private val category: String, private val component: String?, metric: Metric) :
...
internal class InflateTimeMeasurement(metric: Metric) : MetricMeasurementAdapter<RenderResult>(metric) {
  ...
  override fun after(result: RenderResult) =
    if (result.stats.inflateDurationMs != -1L) MetricSample(Instant.now().toEpochMilli(), result.stats.inflateDurationMs)
...
internal class RenderTimeMeasurement(metric: Metric) : MetricMeasurementAdapter<RenderResult>(metric) {
  ...
  override fun after(result: RenderResult) =
    if (result.stats.renderDurationMs != -1L) MetricSample(Instant.now().toEpochMilli(), result.stats.renderDurationMs)
...
internal class ClassLoadTimeMeasurment(metric: Metric) : MetricMeasurementAdapter<RenderResult>(metric) {
  ...
  override fun after(result: RenderResult) =
    if (result.stats.totalClassLoadDurationMs != -1L) MetricSample(Instant.now().toEpochMilli(), result.stats.totalClassLoadDurationMs)
    else null // No render time available
}
...
internal class ClassLoadCountMeasurement(metric: Metric) : MetricMeasurementAdapter<RenderResult>(metric) {
  ...
  override fun after(result: RenderResult) =
    if (result.stats.classesFound != -1L) MetricSample(Instant.now().toEpochMilli(), result.stats.classesFound)
    else null // No render time available
}
...
internal class FirstCallbacksExecutionTimeMeasurement(metric: Metric) : MetricMeasurementAdapter<RenderResult>(metric) {
  ...
  override fun after(result: RenderResult) =
    if (result is ExtendedRenderResult) MetricSample(Instant.now().toEpochMilli(), result.extendedStats.firstExecuteCallbacksDurationMs)
...
internal class FirstTouchEventTimeMeasurement(metric: Metric) : MetricMeasurementAdapter<RenderResult>(metric) {
  ...
  override fun after(result: RenderResult) =
    if (result is ExtendedRenderResult) MetricSample(Instant.now().toEpochMilli(), result.extendedStats.firstInteractionEventDurationMs)
    else null // No time available
}
...
internal class PostTouchEventCallbacksExecutionTimeMeasurement(metric: Metric) : MetricMeasurementAdapter<RenderResult>(metric) {
  ...
  override fun after(result: RenderResult) =
    if (result is ExtendedRenderResult) MetricSample(Instant.now().toEpochMilli(), result.extendedStats.postInteractionEventDurationMs)
    else null // No time available
}
 ...
 *
 * This tracks memory that is not part of the JVM heap but was allocated by layoutlib through JNI. This does not track native memory more
 * generally.
 */
internal class LayoutlibNativeMemoryMeasurement<T>(metric: Metric) : MetricMeasurementAdapter<T>(metric) {
  private var preNativeMemoryUsage = -1L

  override fun before() {
    preNativeMemoryUsage = NativeAllocationRegistry.getMetrics().sumOf { it.nonmallocedBytes + it.mallocedBytes }
  }

  override fun after(result: T) =
    MetricSample(
      Instant.now().toEpochMilli(),
      NativeAllocationRegistry.getMetrics().sumOf { it.nonmallocedBytes + it.mallocedBytes } - preNativeMemoryUsage,
    )
}

@Suppress("UnstableApiUsage") private fun Collection<Long>.median() = Quantiles.median().compute(this)

@Suppress("UnstableApiUsage") private fun Collection<Long>.p95() = Quantiles.percentiles().index(95).compute(this)
...
fun getInflateMetric(task: RenderTask, resultVerifier: (RenderResult) -> Unit): PerfgateRenderMetric {
 ...
 * repetitions and delays could increase the probability of it having any effect, but it is still not guaranteed.
 */
