...
open class ExtendedRenderResult protected constructor(renderResult: RenderResult, val extendedStats: ExtendedStats) :
 ...
 * [firstInteractionEventDurationMs] the duration in milliseconds of the very first call to triggering touch event against the android View
 * and Compose on touch infrastructure. We expect it to take significant time because of class loading that happens the first time this code
 * path is executed. [postInteractionEventDurationMs] the duration in milliseconds of the first call to execute android platform callbacks
 * right after the first touch event is fully propagated. We expect it to take significant time because touch event might add some new
 * callbacks that are executed the very first time and that might load classes.
 */
