...
private suspend fun findDexImpl(debugProcess: DebugProcessImpl, expression: KtElement, location: Location): Result {
  val dexFiles = debugProcess.project.getUserData(DEX_FILES_KEY)
  if (dexFiles != null) {
    return findDexViaFiles(dexFiles, location)
  }
...
private suspend fun findDexViaApkProvider(
  ...
  return apks.firstNotNullOfOrNull { findDexWithLocation(it.apkPath, location) }
}
...
private suspend fun findApksWithExpression(device: IDevice, apkProvider: ApkProvider, expression: KtElement): List<ApkFileUnit> {
  ...
  for (apkInfos in apkProvider.getApks(device)) {
    for (file in apkInfos.files) {
       ...
       * 2. When it fails, the debugger will try to fetch the sought for DEX file via [com.android.tools.deployer.common.Installer]. If a
       *    DEX file is still not found after that, smart step target filtering will simply not be performed, which is not critical for
...
private fun findDexWithLocation(path: Path, location: Location): Dex? {
  val signature = location.declaringType().signature()
  ZipRepo(path).use { zipRepo ->
...
// Retrieve the dex files from the apk(s) generated by the build system
private fun findDexViaFiles(files: List<Path>, location: Location): Result {
  val dex = files.firstNotNullOfOrNull { findDexWithLocation(it, location) }
  val status = if (dex != null) DexSearchStatus.FOUND else DexSearchStatus.DEX_NOT_FOUND
  return Result(dex, status, DexSearchMode.UNSPECIFIED)
}
