...
class AutoProguardMessageRewriterTest {
  ...
  @get:Rule val rule = RuleChain(projectRule, usageTrackerRule, temporaryDirectoryRule, disposableRule)
  ...
  fun rewrite_text() {
    ...
    assertThat(usageTrackerRule.retraceEvents()).containsExactly(stackRetraceEvent(textMapping.fileSize(), isCached = false, TEXT))
  }
  ...
  fun rewrite_partitioned() {
    ...
    assertThat(usageTrackerRule.retraceEvents())
      .containsExactly(stackRetraceEvent(partitionedMapping.fileSize(), isCached = false, PARTITIONED))
  }
  ...
  fun rewrite_noMatchingMapping() {
    ...
    assertThat(usageTrackerRule.retraceEvents()).containsExactly(stackRetraceEvent("MATCHING_MAPPING_NOT_FOUND"))
  }
  ...
  fun rewrite_noMappingId() {
    ...
    assertThat(text).isEqualTo(message)
    assertThat(usageTrackerRule.retraceEvents()).containsExactly(stackRetraceEvent("MAPPINGS_HAVE_NO_MAP_ID"))
  }
...
private fun UsageTrackerRule.retraceEvents() =
  logcatEvents().mapNotNull { it.stackRetrace }.map { it.takeIf { !it.hasRetraceTimeMs() } ?: it.toBuilder().setRetraceTimeMs(0).build() }

private fun stackRetraceEvent(mappingFileSize: Long, isCached: Boolean, mappingType: MappingType): StackRetraceEvent {
