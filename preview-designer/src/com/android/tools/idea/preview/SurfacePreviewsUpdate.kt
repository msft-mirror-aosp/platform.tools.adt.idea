 ...
 * @param refreshFilter a filter to only refresh some of the existing previews. By default, all of them are refreshed
 * @param refreshOrder a function that maps scene managers to integers. Then the previews will be refreshed in ascending order according to
 *   these integers. Also, the order between previews mapped to the same integer is not guaranteed.
...
suspend fun <T : PreviewElement<*>> NlDesignSurface.refreshExistingPreviewElements(
  ...
  previewElementsToSceneManagers.forEachIndexed { index, pair ->
    ...
    renderAndTrack(configureLayoutlibSceneManager(previewElement.displaySettings, sceneManager), refreshEventBuilder)
  }
}
...
suspend fun <T : PsiPreviewElement> NlDesignSurface.updatePreviewsAndRefresh(
  ...
  val (facet, configurationManager) =
    readAction { psiFile.module }
      ...
      ?.let { facet -> facet to ConfigurationManager.getOrCreateInstance(facet.module) } ?: (null to null)
  ...
  val previousOrganizationState = getInstance(project).surfaceState.getOrganizationGroupState(psiFile.virtualFile)

  // Reuse existing groups.
  val groups = elementsToReusableModels.mapNotNull { (_, model) -> model?.organizationGroup }.associateBy { it.groupId }.toMutableMap()
  ...
  val elementsToSceneManagers =
    elementsToReusableModels
      .map { (previewElement, model) ->
        ...
        copySettingsInto(from = previewElement.displaySettings, to = newModel.displaySettings, psiFile)
        ...
        newModel.organizationGroup =
          groups.getOrCreate(previewElement.displaySettings.organizationGroup) { groupId ->
            val displayName = previewElement.displaySettings.organizationName ?: previewElement.displaySettings.name
            val fileAndDisplayName = newModel.displaySettings.fileName.value?.let { "$it.$displayName" } ?: displayName
            OrganizationGroup(
              groupId = groupId,
              displayName = fileAndDisplayName,
              groupType = newModel.displaySettings.groupType.value,
              defaultOpenedState = previousOrganizationState[groupId] ?: newModel.displaySettings.groupType.value.defaultGroupState,
            ) {
              // Everytime state is changed we need to save it.
              isOpened ->
              getInstance(project).surfaceState.saveOrganizationGroupState(psiFile.virtualFile, groupId, isOpened)
            }
          }
  ...
  elementsToSceneManagers.forEachIndexed { idx, (_, sceneManager) ->
    progressIndicator.text = message("refresh.progress.indicator.rendering.preview", idx + 1, elementsToSceneManagers.size)
...
suspend fun <T : PsiPreviewElement> NlDesignSurface.createOrReuseModelForPreviewElement(
  ...
  val newConfiguration: Configuration =
    Configuration.create(configurationManager, FolderConfiguration.createDefault()).also {
      ...
      // reused or new.
      it.setImageTransformation(Configuration.ImageTransformationType.COLOR_BLIND_MODE, this@NlDesignSurface.getGlobalImageTransformation())
    }
  if (modelToReuse != null) {
    ...
    val affinity = calcAffinity(previewElement, previewElementModelAdapter.modelToElement(modelToReuse))
    ...
    newModel =
      NlModel.Builder(parentDisposable, AndroidBuildTargetReference.from(facet, psiFile.virtualFile), file, newConfiguration)
        .withComponentRegistrar(NlComponentRegistrar)
        .withXmlProvider { project, virtualFile ->
          NlModel.getDefaultFile(project, virtualFile).also { it.putUserData(ModuleUtilCore.KEY_MODULE, facet.module) }
        }
        .build()
  }
  return newModel
...
private suspend fun renderAndTrack(sceneManager: LayoutlibSceneManager, refreshEventBuilder: PreviewRefreshEventBuilder?) {
