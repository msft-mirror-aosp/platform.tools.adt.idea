...
class FastPreviewUtilTest {
  ...
  fun `fast compile call`() {
    ...
    runBlocking(Dispatchers.Default) {
      val (result, _) =
        fastCompile(projectRule.testRootDisposable, runReadAction { BuildTargetReference.gradleOnly(testFile.module!!) }, setOf(testFile))
  ...
  fun `fast compile call cancellation`() {
    ...
    val testPreviewManager =
      FastPreviewManager.getTestInstance(projectRule.project, { _, _, _, _ -> blockingDaemon }).also {
      ...
      repeat(50) {
        val job =
          launch(Dispatchers.Default) {
            try {
              val (result, _) = fastCompile(projectRule.testRootDisposable, buildTargetReference, setOf(testFile), testPreviewManager)
  ...
  fun `in-place rename blocks compilation`() = runBlocking {
    ...
    withContext(Dispatchers.EDT) {
      ...
      val fakeEvent = TestActionEvent.createTestEvent(DataManager.getInstance().getDataContext(fixture.editor.component))
    ...
    assertEquals("Renaming in progress", (result as CompilationResult.CompilationAborted).e!!.message)
  }
  ...
  fun `source information is generated by the compose compiler`() {
    ...
    runBlocking {
      ...
      ClassReader(Files.readAllBytes(Path(output).resolve("ComposablesKt.class")))
        .accept(TraceClassVisitor(ClassWriter(ClassWriter.COMPUTE_MAXS), PrintWriter(outputTrace)), ClassReader.EXPAND_FRAMES)
      assertTrue(
        "Source information not generated by the Compose compiler:\n$outputTrace",
        outputTrace.toString().lines().any { it.contains("androidx/compose/runtime/ComposerKt.sourceInformation") },
      )
    }
  }
}
