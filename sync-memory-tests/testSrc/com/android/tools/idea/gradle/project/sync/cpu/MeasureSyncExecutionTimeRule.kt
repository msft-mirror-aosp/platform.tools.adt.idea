...
val CPU_BENCHMARK = Benchmark.Builder("Cpu time").setProject("Android Studio Sync Test").build()
...
class MeasureSyncExecutionTimeRule(
  val syncCount: Int,
  val projectToCompareAgainst: String? = null,
  private val enableAnalyzers: Boolean = true,
) : ExternalResource() {
  ...
  override fun before() {
    ...
    MeasurementPluginConfig.configureAndApply(
      OUTPUT_DIRECTORY,
      captureTypes = setOf(CaptureType.TIMESTAMP),
      CaptureJfrRule.shouldEnableDaemon(),
    )
  }
  ...
  val listener =
    object : GradleSyncListenerWithRoot {
      ...
      override fun syncSucceeded(project: Project, rootProjectPath: @SystemIndependent String) {
        ...
        val androidStartedTimestamp =
          getTimestampForCheckpoint(AndroidMeasurementCheckpoint.ANDROID_STARTED.name) ?: configurationFinishedTimestamp
        val gradleSyncFinishedTimestamp = getTimestampForCheckpoint(MeasurementCheckpoint.SYNC_FINISHED.name)!!
        val androidFinishedTimestamp =
          getTimestampForCheckpoint(AndroidMeasurementCheckpoint.ANDROID_FINISHED.name) ?: gradleSyncFinishedTimestamp
    ...
    results
      .flatMapIndexed { index, value ->
        ...
        listOf(
          ...
          "${prefix}Gradle_Before_Android_Execution_Ms" to
            TimestampedMeasurement(value.finishTimestamp, value.gradleBeforeAndroidExecution, analyzed = false),
          "${prefix}Gradle_Android_Execution_Ms" to
            TimestampedMeasurement(value.finishTimestamp, value.gradleAndroidExecution, analyzed = true),
          "${prefix}Gradle_After_Android_Execution_Ms" to
            TimestampedMeasurement(value.finishTimestamp, value.gradleAfterAndroidExecution, analyzed = false),
      ...
      .forEach { (type, values: List<TimestampedMeasurement>) ->
        values.forEach { value ->
          println(
            "Recording ${projectName}_$type -> ${value.measurement.inWholeMilliseconds} ms (${value.measurement.inWholeSeconds} seconds)"
          )
        }
  ...
  private fun getTimestampForCheckpoint(checkpointName: String): Instant? {
    val file =
      File(OUTPUT_DIRECTORY).walk().firstOrNull { it.nameWithoutExtension.endsWith(checkpointName) && !processedFiles.contains(it.name) }
        ?: return null
    return Instant.fromEpochMilliseconds(file.name.substringBefore('_').toLong()).also { processedFiles.add(file.name) }
  }
