...
/**
 * List of projects available for integration tests. Each project has multiple states for different versions between which we test running
 * upgrades.
 */
enum class TestProject(vararg val path: String) {
...
/**
 * Definition of version pairs available for testing. All versions listed here should directly map to old_agp_test targets in BUILD file. If
 * you add a new version here don't forget to add new target to BUILD. There is no validation of this yet.
 */
...
/**
 * List of available for tests project states. Each state describes which project is it for and which agp version. State can be described as
 * base (all files in project) + patch (only files that change for this state).
 *
...
class UpgradeTestCase(val from: AUATestProjectState, val to: AUATestProjectState) {
  override fun toString(): String {
    return "$from to $to"
  }
}

fun allBaseProjectsForCurrentRunner(): List<AUATestProjectState> =
  AUATestProjectState.values().filter {
    (OldAgpSuite.AGP_VERSION == null || (it.version.agpVersion ?: "LATEST") == OldAgpSuite.AGP_VERSION)
  }
