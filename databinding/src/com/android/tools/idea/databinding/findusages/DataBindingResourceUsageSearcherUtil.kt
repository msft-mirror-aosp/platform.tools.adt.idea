...
@RequiresReadLock
fun getDataBindingSearchTargets(element: PsiElement): List<PsiElement> {
  if (element !is ResourceReferencePsiElement) return emptyList()
  val androidFacet = element.getCopyableUserData(RESOURCE_CONTEXT_ELEMENT)?.androidFacet ?: return emptyList()
  val bindingModuleCache = LayoutBindingModuleCache.getInstance(androidFacet)

  return when (element.resourceReference.resourceType) {
    ResourceType.LAYOUT -> {
      // When searching for usages of a layout resource, we want to add any uses of the
      // generated DataBinding class.
      bindingModuleCache.getLightBindingClasses { layoutGroup -> layoutGroup.mainLayout.resource.name == element.resourceReference.name }
    }
    ResourceType.ID -> {
