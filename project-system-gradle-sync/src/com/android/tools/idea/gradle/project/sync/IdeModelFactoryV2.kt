...
import com.android.tools.idea.gradle.model.IdeUnresolvedKmpAndroidModuleLibraryImpl
...
class IdeModelFactoryV2(private val modelVersions: ModelVersions) {
  ...
  fun moduleLibraryFrom(
    ...
    val artifact: ArtifactRef =
      if (projectInfo.isKmpAndroidComponent()) {
        ...
        library.artifact?.let { NonAndroidAndroidArtifactRef(it) }
          ?: error(
  ...
  private fun Map<String, BuildId>.buildPathToBuildId(projectInfo: ProjectInfo): BuildId {
    ...
    val buildId =
      if (modelVersions[ModelFeature.USES_ABSOLUTE_GRADLE_BUILD_PATHS_IN_DEPENDENCY_MODEL]) {
  ...
  fun ResolvedAndroidProjectPath.resolveVariantName(projectInfo: ProjectInfo, buildId: BuildId): String {
        ...
        projectInfo.productFlavors[dimension]
          ?: run {
            ...
            if (dimension == FAKE_DIMENSION) {
              ...
              projectInfo.attributes["com.android.build.gradle.internal.attributes.VariantAttr"]
 ...
 *
 * It was initially assumed that the Android Gradle plugin should only populate `identifier.artifact` property for non-Android components.
 * However, it happened that it is also not null in the case of dependencies of `androidTest` (or similarly of `TEST_ONLY` projects)
 * artifact on `main` artifacts.
 *
