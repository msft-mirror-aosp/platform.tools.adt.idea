...
data class LibraryIdentity private constructor(val key: Any) {
  companion object {
    @VisibleForTesting
    fun fromLibrary(library: Library): LibraryIdentity =
      LibraryIdentity(
        run {
          library.libraryInfo?.let { "${it.group}:${it.name}:${it.version}${library.artifact?.absolutePath ?: ""}" }
            ?: library.projectInfo?.let { it.displayName }
    ...
    @VisibleForTesting fun fromIdeModel(library: IdeUnresolvedLibrary) = LibraryIdentity(library)
  }

  /**
   * In some cases (explained in the methods below), we prefer one library identity over another to ensure deterministic behavior across
   * multiple syncs. This class tracks which identity belongs to which library to be able to later compare them and sort them according to a
   * pre-defined order.
   */
  class LibraryIdentityProvider {
    ...
    private val comparator =
      compareBy<LibraryIdentity, List<String>>(sortedListComparator) { identityToLibrary[it]?.libraryInfo?.capabilities.orEmpty().sorted() }
        .thenBy(sortedMapEntries) { identityToLibrary[it]?.libraryInfo?.attributes?.entries.orEmpty().sortedBy { it.key } }

    /**
     * For Android libraries, when the build script classpath is different for each project/build, we might end up running the same
     * transform for the same artifact multiple times, and the only signal for that we have on this side is the AAR extraction folder. Here
     * when multiple of those are collapsed together this makes sure we always pick a consistent one across multiple syncs.
     *
     * Function returns identity alongside whether the library is updated to make sure the resulting IDE model can be updated too.
     */
    internal fun associate(library: Library): Pair<LibraryIdentity, Boolean> =
      if (library.type == LibraryType.ANDROID_LIBRARY) {
        ...
        fromLibrary(library).also { identityToLibrary[it] = library } to false // no need to update non-android libraries
      }
    ...
    /**
     * In some cases, artifacts with different set of gradle attributes can end up matching the same artifact, so they end up having the
     * same identity, and we can use any of them. However, we still want to make sure we pick one consistently across multiple syncs. This
     * function is used to sort the library table entries in a deterministic order by comparing their attribute and capability sets.
     */
    fun compare(identity1: LibraryIdentity, identity2: LibraryIdentity): Int = comparator.compare(identity1, identity2)

    companion object {
      private val sortedListComparator =
        Comparator<List<String>> { list1, list2 ->
          for (i in 0 until min(list1.size, list2.size)) {
            ...
            compareBy<String> { it.length }.thenBy { it }.compare(list1[i], list2[i]).let { if (it != 0) return@Comparator it }
          }
          return@Comparator list1.size - list2.size
        }

      private val sortedMapEntries =
        Comparator<List<Map.Entry<String, String>>> { entries1, entries2 ->
          for (i in 0 until min(entries1.size, entries2.size)) {
            compareBy<Map.Entry<String, String>> { it.key }
              .thenBy { it.value }
              .compare(entries1[i], entries2[i])
              .let { if (it != 0) return@Comparator it }
          }
...
class InternedModels(private val buildRootDirectory: File?) {
  ...
  @GuardedBy("lock") private val strings: MutableMap<String, String> = HashMap()
  @GuardedBy("lock") private val libraries: MutableList<IdeUnresolvedLibrary> = mutableListOf()

  // Map from a [LibraryIdentity] to an indexed reference in the library table.
  @GuardedBy("lock") private val keyToLibraryReference = mutableMapOf<LibraryIdentity, LibraryReference>()
  ...
  fun intern(string: String) = lock.withLock { strings.getOrPut(string) { string } }
  ...
  fun internAndroidLibraryV2(v2Library: Library, factory: () -> IdeAndroidLibraryImpl): LibraryReference =
    lock.withLock {
  ...
  fun internJavaLibrary(key: LibraryIdentity, factory: () -> IdeJavaLibraryImpl) = createOrGetLibrary(key, factory)

  fun internJavaLibraryV2(library: Library, factory: () -> IdeJavaLibraryImpl) =
    lock.withLock {
  ...
  fun internUnknownLibraryV2(library: Library, factory: () -> IdeUnresolvedUnknownLibrary) =
    lock.withLock {
      val (identity, _) = libraryIdentityProvider.associate(library)
      createOrGetLibrary(identity, factory)
    }
  ...
  /**
   * Finds an existing or creates a new library instance that match [key]. When creating a new library generates a unique library name based
   * on its artifact address.
   */
  private fun createOrGetLibrary(key: LibraryIdentity, modelFactory: () -> IdeUnresolvedLibrary): LibraryReference {
    return lock.withLock {
      keyToLibraryReference.computeIfAbsent(key) { LibraryReference(libraries.size).also { libraries.add(modelFactory()) } }
    }
  }
  ...
  private fun nameLibraries() {
    data class IdeModelAndReferenceAndIdentity(
      ...
      override fun compareTo(other: IdeModelAndReferenceAndIdentity) = libraryIdentityProvider.compare(this.identity, other.identity)
    }

    keyToLibraryReference.entries
      .distinctBy { it.value }
      .mapNotNull { (key, reference) ->
        (lookup(reference) as? IdeArtifactLibrary)?.let { IdeModelAndReferenceAndIdentity(it, reference, key) }
      }
      .groupBy { convertToLibraryName(it.ideModel.artifactAddress, buildRootDirectory) }
      .forEach { (libraryName, unnamedLibrariesWithSameName) ->
        var counter = 0
        unnamedLibrariesWithSameName.sorted().forEach { (library, reference) ->
  ...
  internal fun addProjectReferenceToArtifactClasspath(id: ClasspathIdentifier, pair: Pair<IdeDependenciesCoreDirect, Int>) {
    lock.withLock { projectReferenceToArtifactClasspathMap.putIfAbsent(id, pair) }
  }

  internal fun getProjectReferenceToArtifactClasspath(classpathId: ClasspathIdentifier) =
    lock.withLock { projectReferenceToArtifactClasspathMap[classpathId] }

  @VisibleForTesting fun getLibraryByKey(libraryIdentity: LibraryIdentity) = lock.withLock { keyToLibraryReference[libraryIdentity] }
}
