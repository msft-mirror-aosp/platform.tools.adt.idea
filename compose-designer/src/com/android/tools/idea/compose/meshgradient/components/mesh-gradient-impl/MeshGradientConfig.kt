 ...
 * @param hasBicubicColor When true, uses Catmull-Rom interpolation for colors, resulting in smoother transitions compared to bilinear
 *   interpolation.
...
internal class MeshGradientConfig(val rows: Int, val columns: Int, @get:JvmName("hasBicubicColor") var hasBicubicColor: Boolean = false) {
   ...
   * based on the point's neighbors.
   */
  ...
  /**
   * Infers bezier control points for a mesh gradient if they are not explicitly specified. This method calculates default control points
   * based on the positions of neighboring points.
   *
   * The inference is done by calculating the normalized vector between neighboring points and scaling it by 1/3rd of the distance between
   * the current point and the neighboring point. For example, the left bezier control point is inferred by calculating the normalized
   * vector between the right and left neighboring points, scaling it by 1/3rd of the distance between the current point and the left
   * neighboring point, and then negating the result. This is done for all four control points.
   */
  private fun inferBezierControlPointsIfRequired() {
    ...
    for (row in 0..rows) {
      for (column in 0..columns) {
        ...
        val leftNeighborIndex = if (column > 0) (row * columnsPlusOne + column - 1) * 2 else index
        ...
        val topNeighborIndex = if (row > 0) ((row - 1) * columnsPlusOne + column) * 2 else index
        ...
        val currentBottomBezierOffset = gradientScopeImpl.bottomBezierOffsets.getOffset(index)
...
private class MeshGradientScopeImpl(override val rows: Int, override val columns: Int) : MeshGradientScope {
   ...
   * @param leftControlPoint The horizontal Bezier control point offset relative to [position] for the edge to the left.
   * @param topControlPoint The vertical Bezier control point offset relative to [position] for the edge above.
   * @param rightControlPoint The horizontal Bezier control point offset relative to [position] for the edge to the right.
   * @param bottomControlPoint The vertical Bezier control point offset relative to [position] for the edge below.
   */
  override fun setVertex(
    ...
    require(row in 0..rows) { "row ($row) must be in range 0..$rows" }
    require(column in 0..columns) { "column ($column) must be in range 0..$columns" }
