...
/**
 * A MeshGradient is a 2D grid of patches defined by vertices. Each vertex has a position, color, and four optional Bezier control points
 * (tangents) that define the curvature of the edges connecting neighboring vertices.
 *
 * **Grid Dimensions:** For a given [rows] and [columns] (representing the number of patches), there are a total of `(rows + 1) * (columns +
 * 1)` vertices. For example, a 1x1 mesh consists of 4 vertices forming a single rectangular patch.
 *
 * **Coordinate System:** All positions and Bezier offsets use a **normalized coordinate system** where (0,0) is the top-left and (1,1) is
 * the bottom-right of the drawing bounds.
 *
...
constructor(
  ...
  override fun toString(): String = "MeshGradientPainter(rows: $rows, columns: $columns, hasBicubicColor: $hasBicubicColor)"
}
