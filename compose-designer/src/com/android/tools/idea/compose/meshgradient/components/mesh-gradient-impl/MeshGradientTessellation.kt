...
/**
 * Calculates the flat index into a vertex-based array (like positions or colors) based on the [row] and [col] in a grid with a specific
 * number of [columns].
 *
 ...
 */
internal fun calculateMeshGradientSubdivisions(rows: Int, columns: Int, positions: FloatArray, size: Size): IntSize {
  var maxW = 0f
  var maxH = 0f
  ...
  val subdivisionsU = ceil(maxW / TargetPxPerSegment).toInt().coerceIn(MinSubdivision, MaxSubdivision)
  val subdivisionsV = ceil(maxH / TargetPxPerSegment).toInt().coerceIn(MinSubdivision, MaxSubdivision)
