...
fun createUrlClassLoader(paths: List<Path>, allowLock: Boolean = !SystemInfo.isWindows): UrlClassLoader {
  return UrlClassLoader.build().files(paths).useCache(ourLoaderCachePool) { true }.allowLock(allowLock).get()
}

private val additionalLibraries: List<Path>
  get() {
    val layoutlibDistributionPath =
      StudioEmbeddedRenderTarget.ourEmbeddedLayoutlibPath ?: return emptyList() // Error is already logged by getEmbeddedLayoutLibPath
 ...
 * packages defined in [StudioModuleClassLoader.PACKAGES_TO_RENAME].
 */
internal const val INTERNAL_PACKAGE = "_layoutlib_._internal_."

/**
 * Method uses to remap type names using [StudioModuleClassLoader.INTERNAL_PACKAGE] as prefix to its original name so they original class
 * can be loaded from the file system correctly.
 */
 ...
 */
internal class ModuleClassLoaderImpl(
...
) : DelegatingClassLoader.Loader, Disposable {
  ...
  /**
   * TODO(b/445923198): delete timestamp variable once lifecycle race condition is fixed. Timestamp when [dispose] was first called, or null
   *   if [dispose] has not been called yet. This is used to avoid logging many warnings due to an existing race condition. See [loadClass]
   *   for more details.
   */
  ...
  /**
   * List of libraries used in this [ModuleClassLoaderImpl]. For common source sets, external libraries required for rendering the preview
   * might come from the Android counterpart. Hence, we are adding them here.
   */
  ...
  val nonProjectLoadedClassNames: Set<String>
    get() = _nonProjectLoadedClassNames
  ...
  private fun createProjectLoader(
    ...
    onClassRewrite: (String, Long, Int) -> Unit,
  ) =
    AsmTransformingLoader(
      projectTransforms,
      ListeningLoader(loader, onAfterLoad = { fqcn, _ -> _projectLoadedClassNames.add(fqcn) }),
      PseudoClassLocatorForLoader(listOfNotNull(projectSystemLoader, dependenciesLoader, parentLoader).asSequence(), parentClassLoader),
      ClassWriter.COMPUTE_FRAMES,
      onClassRewrite,
    )

  private fun createNonProjectLoader(
  ...
  init {
    ...
    val allLoaders = listOfNotNull(projectLoader, nonProjectLoader, RecyclerViewAdapterLoader())
    loader = MultiLoaderWithAffinity(allLoaders)
  }

  /** Creates an overlay loader. See [OverlayLoader]. */
  private fun createOptionalOverlayLoader(
    dependenciesLoader: DelegatingClassLoader.Loader?,
    onClassRewrite: (String, Long, Int) -> Unit,
  ): DelegatingClassLoader.Loader {
