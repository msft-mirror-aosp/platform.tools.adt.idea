...
/**
 * This class represent a Kotlin source file and the location (memory or file) of the compiled bytecode that can be used to be injected into
 * the inline cache of the code generation process.
 */
data class SourceInlineCandidate(val sourceFile: KtFile, val className: String) {
  ...
  /**
   * Return true if we can populate the KtFile's inline cache entry of the code generation process. IE: Can we have the bytecode available
   * either in memory or on disk somehow.
   */
  ...
  /**
   * Replaces the current cache with new bytecode. This should be invoked when Live Edit finished compiling a KtFile and the on disk build
 ...
 *
 * Note that this function is NOT recursive and only compute inline functions of one depth level. The returned KtFiles themselves can also
 * reference inline functions from another source file that are not part of the return set.
 */
fun analyzeSingleDepthInlinedFunctions(
  file: KtFile,
  bindingContext: BindingContext,
  cache: SourceInlineCandidateCache,
): Set<SourceInlineCandidate> {
...
private fun analyzeElementWithOneLevelInline(
  ...
  for (declaration in declarationsWithBody) {
    ...
    if (containingClass == null) {
      ...
      if (element != file) {
        requestedClasses.add(cache.computeIfAbsent(name) { SourceInlineCandidate(file, it) })
      }
    } else {
