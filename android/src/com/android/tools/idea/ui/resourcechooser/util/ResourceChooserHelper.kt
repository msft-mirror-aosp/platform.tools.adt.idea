 ...
 * [resourceTypes].
 *
 * Selecting a resource in the ResourcePicker will populate [ResourcePickerDialog.resourceName] in the following format: '@string/my_string'
 * or '?attr/my_attribute' for theme attributes, the resource name will include the appropriate namespace.
 *
...
fun createAndShowColorPickerPopup(
  ...
  val onPopupClosed = { Disposer.dispose(disposable) }
  val popupDialog = LightCalloutPopup(onPopupClosed, onPopupClosed, null)

  val colorPicker =
    if (colorPickedCallback == null) null
    else {
  ...
  val resourcePicker =
    if (
      facet != null && colorPickedCallback != null && contextFile != null && resourceResolver != null && colorResourcePickedCallback != null
    ) {
 ...
 * @param locationToShow Preferred location in the screen to show the popup dialog, if null, the current location of the mouse will be used
 * @param resourcePickedCallback The callback for whenever a resource is selected in the ResourcePicker, returns the string reference of the
 *   resource Eg: @color/colorPrimary
 */
