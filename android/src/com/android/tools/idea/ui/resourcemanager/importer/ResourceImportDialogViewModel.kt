...
class ResourceImportDialogViewModel(
   ...
   *
   * They are stored in an IdentitySet because there might be conflicts when a [ResourceAssetSet] is being renamed with a name similar to
   * another [ResourceAssetSet] being imported.
   */
  private val assetSetsToImport = assets.take(MAX_IMPORT_FILES).groupIntoDesignAssetSet().toIdentitySet()
  ...
  val fileCount: Int
    get() = assetSets.sumOf { it.assets.size }
  ...
  fun getAssetPreview(asset: DesignAsset): CompletableFuture<out Image?> {
    return rendererManager.getViewer(asset.file).getImage(asset.file, facet.module, JBUI.size(50))
  }

  fun getItemNumberString(assetSet: ResourceAssetSet) = "(${assetSet.assets.size} ${StringUtil.pluralize("item", assetSet.assets.size)})"
  ...
  /**
   * Invoke a path chooser and add new files to the list of assets to import. If a file is already present, it won't be added and new
   * [DesignAsset] will be merged with [ResourceAssetSet] of the same name.
   *
   * [assetAddedCallback] will be called with a new or existing [ResourceAssetSet] and a list of newly added [DesignAsset]s, which means
   * that it's a subset of [ResourceAssetSet.assets]. This allows the view to merge the new [DesignAsset] within a potential existing view
   * of a [ResourceAssetSet]. The callback won't be called if there is no new file.
   */
  fun importMoreAssets(assetAddedCallback: (ResourceAssetSet, List<DesignAsset>) -> Unit) {
    ...
    chooseDesignAssets(importersProvider) { newAssetSets ->
      newAssetSets.take(MAX_IMPORT_FILES).groupIntoDesignAssetSet().forEach { addAssetSet(assetByName, it, assetAddedCallback) }
    }
  }
  ...
  /**
   * Add the [assetSet] to the list of asset to be imported. If [existingAssets] contains a [ResourceAssetSet] with the same name as
   * [assetSet], the [assetSet] will be added the existing [ResourceAssetSet], otherwise a new one will be created.
   *
   * @param existingAssets A map from a [ResourceAssetSet]'s name to the [ResourceAssetSet]. This is used to avoid iterating through the
   *   whole [assetSetsToImport] set to try find a [ResourceAssetSet] with the same name.
   */
  ...
  /**
   * Creates a copy of [assetSet] with [newName] set as the [ResourceAssetSet]'s name. This method does not modify the underlying
   * [DesignAsset], which is just passed to the newly created [ResourceAssetSet].
   *
   * [assetRenamedCallback] is a callback with the old [assetSet] name and the newly created [ResourceAssetSet]. This meant to be used by
   * the view to update itself when it is holding a map from view to [ResourceAssetSet].
   */
  fun rename(assetSet: ResourceAssetSet, newName: String, assetRenamedCallback: (newAssetSet: ResourceAssetSet) -> Unit) {
   ...
   *
   * To let the [FileImportRowViewModel] delete itself, a callback needs to be provided to notify its owner that it has been deleted, and
   * the view needs to be updated.
   */
  fun createFileViewModel(asset: DesignAsset, removeCallback: (DesignAsset) -> Unit): FileImportRowViewModel {
  ...
  private fun getSameNameIsImportedValidationInfo(field: JTextField?) =
    ValidationInfo("A resource with the same name is also being imported.", field).asWarning()

  private fun checkIfNameUnique(newName: String?): Boolean {
    var nameSeen = false
    return assetSetsToImport.any {
      if (it.name == newName) {
