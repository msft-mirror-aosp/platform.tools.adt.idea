...
private val logger: Logger
  get() = logger<PathValidator>()

/**
 * A class which contains validation logic that should be run on a path to ensure that there won't be any problems with using it. If there
 * is an issue, we offer a readable string which we can show to the user.
 *
...
class PathValidator
/**
 * Constructs a class that will validate a path against the various passed in rules, returning a readable message if something goes wrong. A
 * name describing the purpose of the path should be included as it will be used in the error messages when applicable.
 */
private constructor(val pathName: String, @get:TestOnly val errors: Iterable<Rule>, private val warnings: Iterable<Rule>) :
  Validator<Path> {
  ...
  override fun validate(value: Path): Result =
    try {
      validate(value, Severity.ERROR).takeIf { it != Result.OK }
        ?: validate(value, Severity.WARNING).takeIf { it != Result.OK }
        ?: Result.OK
    } catch (ex: Exception) {
      logger.warn(ex)
  ...
  companion object {
     ...
     */
    @JvmStatic fun createDefault(pathName: String): PathValidator = Builder().withAllRules().build(pathName)

    @JvmStatic fun forAndroidSdkLocation(): PathValidator = Builder().withCommonRules().build("Android SDK location")

    @JvmStatic fun forAndroidNdkLocation(): PathValidator = Builder().withCommonRules().build("Android NDK location")
  }
}
...
interface Rule {
   ...
   *
   * If a file is returned, it will usually be the same as `file` but not always. For example, a rule may complain about a file's parent.
   */
...
private fun createSimpleRule(matches: (Path) -> Boolean, getMessage: (Path, String) -> String) =
  object : Rule {
...
private fun createRecursiveRule(matches: (Path) -> Boolean, getMessage: (Path, String) -> String) =
  object : Rule {
...
private val RESERVED_WINDOWS_FILENAMES: Set<String> =
  setOf(
    "con",
    "prn",
    "aux",
...
/**
 * Although Android Studio and Gradle support paths longer than MAX_PATH (260) on Windows, there is still a limit to the length of the path
 * to the root directory, as each project directory contains a "gradlew.bat" file (used to build the project), and the length of the path to
 * the "gradlew.bat" cannot exceed MAX_PATH, because Windows limits path of executables to MAX_PATH (for backward compatibility reasons).
 */
 ...
 */
fun filenameRule(description: String, filenameIsAllowed: (String) -> Boolean) =
  createSimpleRule(
    { path -> !filenameIsAllowed(path.fileName?.toString() ?: "") },
    { _, fieldName -> "The $fieldName specified $description." },
  )

val IS_EMPTY = createSimpleRule({ path -> path.fileName?.toString().isNullOrEmpty() }, { _, fieldName -> "Please specify a $fieldName." })
...
val LOCATION_IS_A_FILE = createSimpleRule(CancellableFileIo::isRegularFile, { _, fieldName -> "The $fieldName specified already exists." })

val LOCATION_IS_NOT_A_FILE =
  createSimpleRule({ path -> !CancellableFileIo.isRegularFile(path) }, { _, fieldName -> "The $fieldName specified does not exist." })

val LOCATION_IS_ROOT =
  createSimpleRule({ path -> path.parent == null }, { _, fieldName -> "The $fieldName cannot be at the filesystem root." })

val PARENT_IS_NOT_A_DIRECTORY =
  createSimpleRule(
    { path ->
...
val PATH_INSIDE_ANDROID_STUDIO =
  createSimpleRule(
...
val PARENT_DIRECTORY_NOT_WRITABLE =
  createRecursiveRule(
    { path ->
      val parent = path.parent
