...
class GenericFileFilterTest {
  ...
  fun setUp() {
    whenever(localFileSystem.findFileByPathIfCached(ArgumentMatchers.anyString())).thenAnswer { invocation ->
      ...
      mock(VirtualFile::class.java).apply { whenever(this.path).thenReturn(pathString) }
    }
  }
  ...
  @Test
  fun `recognize simple Linux path`() =
  ...
  @Test
  fun `recognize simple Windows path`() =
    getFilterResultAndCheckHighlightPositions(
        """
        | C:\path\to\file
          ^^^^^^^^^^^^^^^
        """
          .trimIndent(),
        listOf("""C:\path\to\file"""),
      )
      .checkFileLinks("""C:\path\to\file""")

  @Test
  fun `recognize simple Windows path with forward slashes`() =
    getFilterResultAndCheckHighlightPositions(
        """
        | C:/path/to/file
          ^^^^^^^^^^^^^^^
        """
          .trimIndent(),
        listOf("""C:/path/to/file"""),
      )
      .checkFileLinks("""C:/path/to/file""")

  @Test
  fun `recognize path with line number`() =
    getFilterResultAndCheckHighlightPositions(
        """
        | /path/to/file.c:3
          ^^^^^^^^^^^^^^^^^
        """
          .trimIndent(),
        listOf("/path/to/file.c"),
      )
      .checkFileLinks("/path/to/file.c")

  @Test
  fun `a huge line number will not break it`() =
    getFilterResultAndCheckHighlightPositions(
        """
  ...
  @Test
  fun `recognize path with line number and column`() =
    getFilterResultAndCheckHighlightPositions(
        """
        | /path/to/file.c:3:7
          ^^^^^^^^^^^^^^^^^^^
  ...
  @Test
  fun `recognize path in middle of a line`() =
    getFilterResultAndCheckHighlightPositions(
        """
  ...
  @Test
  fun `recognize multiple paths in a line`() =
    getFilterResultAndCheckHighlightPositions(
        """
        ...
        """
          .trimIndent(),
        listOf("/path/to/file", "/another/path/to/file"),
      )
      .checkFileLinks("/path/to/file", "/another/path/to/file")

  @Test
  fun `recognize path with line number and column in parenthesis`() =
    getFilterResultAndCheckHighlightPositions(
        """
  ...
  fun `recognize path with space`() =
    getFilterResultAndCheckHighlightPositions(
        ...
        """
          .trimIndent(),
        listOf("/path/to/file/with space"),
      )
  ...
  fun `recognize path with space and numbers parenthesis Windows`() =
    getFilterResultAndCheckHighlightPositions(
        ...
        """
          .trimIndent(),
        listOf("""C:\path\to\file\with space.kt"""),
      )
      .checkFileLinks("""C:\path\to\file\with space.kt""")

  @Test
  fun `multiple lines and multiple links`() =
    getFilterResultAndCheckHighlightPositions(
        """
        ...
        """
          .trimIndent(),
        listOf("/path/to/file1", "/path/to/file2", "/path/to/file3", """C:\path\to\file4"""),
      )
      .checkFileLinks("/path/to/file1", "/path/to/file2", "/path/to/file3", """C:\path\to\file4""")
  ...
  fun `skip files not cached by local file system`() =
    getFilterResultAndCheckHighlightPositions(
        ...
        """
          .trimIndent(),
        listOf("/path/to/cached/file"),
      )
      .checkFileLinks("/path/to/cached/file")
  ...
  @Test
  fun `take longest path when prefix with spaces exists`() =
    getFilterResultAndCheckHighlightPositions(
        """
        ...
        """
          .trimIndent(),
        listOf("""C:\path\to\file\with space\and no more.kt""", """C:\path\to\file\with space and more.kt"""),
      )
      .checkFileLinks("""C:\path\to\file\with space\and no more.kt""", """C:\path\to\file\with space and more.kt""")
  ...
  @Test
  fun `real world case for bug 167701951`() =
    getFilterResultAndCheckHighlightPositions(
        """
  ...
  fun `fuzz test`() {
    ...
    val linuxPathGen = ('/' + wordGen).repeated(2..10).useGenerated { allGeneratedPaths += it }
    val windowsPathGen = (oneOf('A'..'Z') + ":" + ('\\' + wordGen).repeated(2..10)).useGenerated { allGeneratedPaths += it }
    ...
    val sentenceGen =
      oneOf(wordGen.repeated(1..10, " "), oneOf(linuxPathGen, windowsPathGen, pathWithLineNumberGen, pathWithLineAndColumnNumberGen))
        .repeated(1..5, " ")
  ...
  private fun getFilterResultAndCheckHighlightPositions(
    ...
    whenever(
        localFileSystem.findFileByPathIfCached(
          ArgumentMatchers.argThat { arg -> !validPaths.any { arg == it || it.startsWith("$arg/") || it.startsWith("$arg\\") } }
        )
      )
      .thenReturn(null)
    content.lines().forEach { line ->
      if (!checkHighlights || line.startsWith('|')) {
        ...
        val actualHighlighted =
          results.last()!!.resultItems.map { item ->
            previousInputLine.substring(
              item.getHighlightStartOffset() - previousInputStartIndex,
