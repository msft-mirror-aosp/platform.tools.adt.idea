...
public class MigrateToAppCompatGradleTest extends AndroidGradleTestCase {
  ...
  public void testMigrateOnAndroidXProject2() throws Exception {
    ...
    WriteAction.run(() -> PsiManager.getInstance(getProject()).reloadFromDisk(PsiUtilCore.getPsiFile(getProject(), mainActivityFile)));
    mainActivityKt = FileDocumentManager.getInstance().getDocument(mainActivityFile).getText();
