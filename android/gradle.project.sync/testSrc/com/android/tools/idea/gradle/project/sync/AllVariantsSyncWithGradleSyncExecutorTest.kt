...
/**
 * Tests for AllVariantsSync Sync option. The tests run Sync using both V1 and V2. We first run Sync using the normal process, and verify
 * that we only request one variant per gradle project. The second step is to run Sync request (fetch gradle models) using
 * GradleSyncExecutor (used in the PSD workflow), and verify that we get all the gradle project's variants created.
 *
...
class AllVariantsSyncWithGradleSyncExecutorTest : SnapshotComparisonTest {
  ...
  fun testAllVariantSyncWithV2() {
    ...
    preparedProject.open { project -> runSvsAndAvsSyncAndVerifyFetchedVariants(project, 12) }
  }
  ...
  fun testAllVariantWithBuildSrc() {
    ...
    preparedProject.open { project -> runSvsAndAvsSyncAndVerifyFetchedVariants(project, 2) }
  }
  ...
  private fun runSvsAndAvsSyncAndVerifyFetchedVariants(project: Project, variantsExpectedInModule0: Int) {
    ...
    val gradleModules =
      ProgressManager.getInstance()
        .runProcessWithProgressSynchronously(
          ThrowableComputable { syncExecutor.fetchGradleModels() },
          "Test Run AllVariantsSync",
          false,
          project,
        )
    ...
    dumper.dumpAllVariantsSyncAndroidModuleModel(
      project,
      GradleAndroidDependencyModel.createWithAllVariants(allVariantsSyncAndroidModel, fromLibraryTables(gradleModules.libraries!!, null)),
      project.basePath!!,
    )
