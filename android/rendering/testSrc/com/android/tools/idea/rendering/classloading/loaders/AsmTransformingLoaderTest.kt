...
class AsmTransformingLoaderTest {
  ...
  fun `check transformation is applied`() {
    val methodRenamer =
      SimpleRemapper("com/android/tools/idea/rendering/classloading/loaders/TransformableClass.methodA()V", "renamedMethodA")

    val staticLoader = StaticLoader(TransformableClass::class.java.name to loadClassBytes(TransformableClass::class.java))

    val transformLoader =
      AsmTransformingLoader(
        ClassTransform(listOf(Function<ClassVisitor, ClassVisitor> { visitor -> ClassRemapper(visitor, methodRenamer) })),
        staticLoader,
        NopClassLocator,
      )
    ...
    assertEquals(
      ...
      textifyClass(transformedClass!!)
        .lines()
        .map { it.trim() }
        .filter { it.contains("public final") && it.contains("method", true) }
        .joinToString("\n"),
    )
  }
  ...
  fun `check null classes do not throw exceptions`() {
    val transformLoader =
      AsmTransformingLoader(
        ClassTransform(listOf(Function<ClassVisitor, ClassVisitor> { throw IllegalStateException("Should not be called") })),
        NopLoader,
        NopClassLocator,
      )
