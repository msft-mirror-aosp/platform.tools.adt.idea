...
const val BASELINE_PROFILES_PLUGIN_MIN_REV = "1.5.0-beta01"
...
fun RecipeExecutor.createTestClasses(
  ...
  val (generatorContent, benchmarksContent) =
    when (language) {
      Language.Kotlin -> {
        // Create Baseline Profile Generator class
        val generatorContent =
          baselineProfileGeneratorKt(
...
fun setupRunConfigurations(targetModule: Module, runManager: RunManager = RunManager.getInstance(targetModule.project)) {
  ...
  val runConfigForSelected =
    AndroidBaselineProfileRunConfiguration(
        project = project,
...
fun Module.getModuleNameForGradleTask(): String {
  ...
  // module name contains also the whole project name,
  // so we need to remove it, so we can use it to a Gradle task
