...
class AndroidApiLevelComboBox : JComboBox<VersionItem?>() {
  ...
  fun init(newFormFactor: FormFactor, items: List<VersionItem>) {
    ...
    // Try to keep the old selection. If not possible (or no previous selection), use the last saved
    // selection. If the form factor changes we want to prevent keeping the old selection and want
    // to ensure the last saved selection for the new form factor is used instead.
