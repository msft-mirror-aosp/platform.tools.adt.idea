...
class RecyclablePooledImage(var backingImage: RecyclableImage?) : ImagePool.Image, DisposableImage {
  ...
  override fun drawImageTo(g: Graphics, dx1: Int, dy1: Int, dx2: Int, dy2: Int, sx1: Int, sy1: Int, sx2: Int, sy2: Int) {
    ...
    if (isDisposed.get() || recyclableImage == null) {
      LOG.warn("drawImageTo called on disposed RecyclablePooledImage")
      return
    }
  ...
  override fun paint(command: Consumer<Graphics2D?>) {
    val recyclableImage = backingImage
    if (isDisposed.get() || recyclableImage == null) {
      LOG.warn("paint called on disposed RecyclablePooledImage")
      return
    }
  ...
  override fun getCopy(x: Int, y: Int, w: Int, h: Int): BufferedImage? {
    val recyclableImage = backingImage
    if (isDisposed.get() || recyclableImage == null) {
      LOG.warn("getCopy called on disposed RecyclablePooledImage")
      return null
    }
  ...
  companion object {
    private val LOG = Logger.getInstance(RecyclablePooledImage::class.java)
  }
}
