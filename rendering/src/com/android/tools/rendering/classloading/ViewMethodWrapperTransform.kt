...
private val ERROR_METHOD_DESCRIPTION: String? =
  try {
...
class ViewMethodWrapperTransform(delegate: ClassVisitor) : ClassVisitor(Opcodes.ASM9, delegate), ClassVisitorUniqueIdProvider {
  ...
  private fun wrapMethod(access: Int, name: String, desc: String, signature: String?, exceptions: Array<String>?) {
    ...
    val argumentTypes = Type.getMethodType(desc).argumentTypes
    var nLocals = 1
    for (argType in argumentTypes) {
      mw.visitVarInsn(argType.getOpcode(Opcodes.ILOAD), nLocals++)
    }
    mw.visitMethodInsn(Opcodes.INVOKEVIRTUAL, currentClassName, name + ORIGINAL_SUFFIX, desc, false)
