...
void Agent::Initialize(const vector<string>& args) {
  ...
  device_type_ = HasBuildCharacteristic("glasses", build_characteristics) ? DeviceType::GLASSES :
                 HasBuildCharacteristic("watch", build_characteristics) ? DeviceType::WATCH :
