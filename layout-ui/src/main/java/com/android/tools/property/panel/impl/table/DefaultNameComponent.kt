...
class DefaultNameComponent(private val tableSupport: TableSupport? = null) : JPanel(BorderLayout()) {
  ...
  override fun updateUI() {
    ...
    expandedWhiteIcon =
      if (NewUI.isEnabled()) UIUtil.getTreeExpandedIcon() else ColoredIconGenerator.generateWhiteIcon(UIUtil.getTreeExpandedIcon())
    collapsedWhiteIcon =
      if (NewUI.isEnabled()) UIUtil.getTreeCollapsedIcon() else ColoredIconGenerator.generateWhiteIcon(UIUtil.getTreeCollapsedIcon())
  ...
  fun setUpItem(table: PTable, item: PTableItem, depth: Int, isSelected: Boolean, hasFocus: Boolean, isExpanded: Boolean): JComponent {
    ...
    when {
      ...
      item is PropertyItem && item.namespaceIcon != null -> {
        iconLabel.icon =
          item.namespaceIcon?.let { if (isSelected && hasFocus && !NewUI.isEnabled()) ColoredIconGenerator.generateWhiteIcon(it) else it }
      }
      else -> {
        iconLabel.icon = null
      }
