...
class LeakCanaryScreenTest : WithFakeTimer {
  ...
  @Rule @JvmField val grpcChannel = FakeGrpcChannel("LeakCanaryScreenTestChannel", transportService)
  ...
  fun `test leak canary left and right panel data changes on the selected leak changes`() {
    ...
    composeTestRule
      .onNodeWithText("mainActivity instance of com.amaze.filemanager.ui.activities." + "MainActivity with mDestroyed = false")
      .isDisplayed()
    composeTestRule.onNodeWithText("Leaking").isDisplayed()
    composeTestRule.onNodeWithText("No").isDisplayed()
  }
  ...
  fun `test leak canary left and right panel data with leaks having className in multiple lines`() {
    ...
    // By default, first leak will be displayed in the leak details panel
    composeTestRule
      .onNodeWithTag("com.google.android.apps.chromecast.app.widget.recyclerview.BackgroundClickableRecyclerView")
      .isDisplayed()
  ...
  fun `test expand and collapse all via shortcuts`() {
    ...
    focusNode.performKeyInput { withKeyDown(Key.CtrlLeft) { pressKey(Key.NumPadAdd) } }

    // Now, no nodes are closed.
    composeTestRule.onAllNodesWithContentDescription(TaskBasedUxStrings.LEAKCANARY_CLOSE).assertCountEquals(0)

    // Send Ctrl + Minus shortcut to collapse all.
    focusNode.performKeyInput { withKeyDown(Key.CtrlLeft) { pressKey(Key.Minus) } }
