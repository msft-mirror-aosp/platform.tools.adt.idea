    ...
    object : JPanel(BorderLayout()) {
        ...
        private fun draw(g2d: Graphics2D) {
          ...
          val startIndexInclusive =
            when {
              clipRect != null && clipRect.x != 0 -> overlapIndex(clipRect.x / scaleX, 0f).let { if (it < 0) -(it + 1) else it }
              else -> 0
            }
          val endIndexExclusive =
            when {
              clipRect != null && clipRect.width != width ->
                overlapIndex((clipRect.x + clipRect.width) / scaleX, 0f).let {
...
private tailrec fun Container.getOpaqueContainer(): Container =
  when {
