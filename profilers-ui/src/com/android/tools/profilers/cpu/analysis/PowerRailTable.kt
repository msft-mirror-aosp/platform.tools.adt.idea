...
/**
 * UI component for a table presenting the average power and cumulative energy consumption per power rail in the user-selected range. Takes
 * a map of power rail names to their respective [PowerCounterData] and generates [PowerRailRow]s. The table will look like this: | Power
 * Rail | Average power (mW) | Cumulative consumption (µWs) | | Foo | 20.0 | 123,456,789 | | Bar | 50.0 | 987,654,321 |
 *
...
class PowerRailTable(
  ...
  init {
    ...
    table.tableHeader.columnModel.getColumn(Column.CONSUMPTION.ordinal).headerRenderer =
      TableCellRenderer { table, value, isSelected, hasFocus, row, column ->
        val headerComponent: Component =
          table.tableHeader.defaultRenderer.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column)
        JLabel().apply {
    ...
    val timeRange =
      SummaryDetailsViewBase.formatTimeRangeAsString(
        selectionRange = selectionRange,
        relativeZeroPoint = captureRange.min.toLong(),
        separator = '→',
      )
    val titleWithTimeRange = "<b>$title</b> (Range: $timeRange)"
    component =
      HideablePanel.Builder(titleWithTimeRange, tableContainer)
...
private data class PowerRailRow(val name: String, val consumption: Long, val average: Double)
