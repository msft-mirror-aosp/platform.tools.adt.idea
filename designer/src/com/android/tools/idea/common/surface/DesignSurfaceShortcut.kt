 ...
 * @param keyChar KeyChar for the shortcut. This is optional and used to register the the shortcut when the key code does not correspond to
 *   the key char (for example, the '+' in on the KeyCode of '='. This is necessary for the Intellij ActionButton which uses the key code to
 *   display the character in the tooltip and not the key char.
 *
...
enum class DesignSurfaceShortcut(val keyCode: Int, private val keyChar: Char? = null) {
  ...
  fun registerForAction(shortcutAction: AnAction, component: JComponent, parentDisposable: Disposable): AnAction {
    shortcutAction.registerCustomShortcutSet(shortcutSet, component, parentDisposable)
    return shortcutAction
  }

  /**
   * Register this shortcut on [shortcutAction] within the provided [component] and display the shortcut hint in the description of
   * [visibleAction]. This is useful if the action for which the shortcut is registered is in a submenu.
   *
  ...
  fun registerForHiddenAction(
    ...
    presentation.description = presentation.description + " (" + KeymapUtil.getShortcutsText(shortcutSet.shortcuts) + ")"
    return visibleAction
  }
}
