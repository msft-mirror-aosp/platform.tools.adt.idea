 ...
 *
 * Given a list of [NlComponent]s generate a table with all the properties the components have in common. Only attributes that are available
 * for all versions down to and including minSdkVersion will be included.
 *
 * If layoutlib inflated a different view class, add the attributes from that view as well (unless an existing framework attribute exists).
 *
 * There are special exceptions for the srcCompat attribute and attributes on an AutoCompleteTextView widget.
 */
  ...
  private class PropertiesGenerator(
    ...
    fun generate(): Table<String, String, NlPropertyItem> {
      ...
      val commonJustAndroidValue =
        common.filter { combinedProperties[ANDROID_URI, it]?.value != null && combinedProperties[AUTO_URI, it]?.value == null }.toSet()
    ...
    private fun removeEmptyProperty(properties: Table<String, String, NlPropertyItem>, namespace: String, name: String) {
    ...
    private fun loadPropertiesFromDescriptors(tag: XmlTag, descriptors: Array<XmlAttributeDescriptor>, componentClass: PsiClass?) {
      for (desc in descriptors) {
        ...
        if (ANDROID_URI == namespaceUri && apiLookup != null && apiLookup.getFieldVersions("android/R\$attr", name).min() > minApi) {
          continue
        }
     ...
     * @param componentClass the top level class we are loading attributes for. This method may be called multiple times with once for each
     *   of the super classes of this component.
     */
    ...
    private fun addPropertyFromAttribute(attribute: AttributeDefinition, psiClass: PsiClass, componentClass: PsiClass?) {
      ...
      val property =
        createProperty(namespace, attribute.name, attribute, componentClass, psiClass.qualifiedName ?: "", model, components) ?: return
      if (ANDROID_URI == namespace && apiLookup != null && apiLookup.getFieldVersions("android/R\$attr", attribute.name).min() > minApi) {
        // Exclude the framework attributes that were added after the current min API level.
        return
      }
