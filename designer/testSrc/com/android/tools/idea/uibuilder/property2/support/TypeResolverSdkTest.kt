...
class TypeResolverSdkTest {
  ...
  private fun checkViewAttributes(tagName: String, report: Report) {
    ...
    val tag = getTag(util.components.first())!!
  ...
  private fun checkViewLayoutAttributes(tagName: String, report: Report) {
    ...
    val tag = getTag(util.components.first().parent!!)!!
    checkAttributes(tag, report)
  }

  private fun getTag(component: NlComponent): XmlTag? {
    val tag = component.backend.getTagPointer().element
    return if (tag?.isValid == true) tag else null
  }
