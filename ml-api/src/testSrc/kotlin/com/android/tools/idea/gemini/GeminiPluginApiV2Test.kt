...
private class FakeGeminiPluginApiV2 : GeminiPluginApiV2 {
  ...
  var toolWindowFileReferences: List<java.nio.file.Path> = emptyList()
  ...
  override suspend fun submitQueryInToolWindow(
    ...
    query: String,
    blobs: List<LlmBlob>,
    fileReferences: List<java.nio.file.Path>,
    conversationTarget: LlmConversationTarget,
  ): LlmChatInToolWindowResult {
    this.toolWindowQuery = query
    this.editorBlobs = blobs
    this.toolWindowFileReferences = fileReferences
    this.conversationTarget = conversationTarget
    return LlmChatInToolWindowResult.Success("conv-id")
  }
}
...
class GeminiPluginApiV2Test : BasePlatformTestCase() {
  ...
  fun defaultInstance_returnsFallback() = runBlocking {
    ...
    val toolWindowWithPayloadsResult = instance.submitQueryWithPayloadsInToolWindow(project = project, query = "hi")
    assertThat(toolWindowWithPayloadsResult).isInstanceOf(LlmChatInToolWindowResult.RequestNotSubmitted::class.java)
    assertThat((toolWindowWithPayloadsResult as LlmChatInToolWindowResult.RequestNotSubmitted).reason)
      .isEqualTo(LlmFailureReason.NO_MODELS_AVAILABLE)
  }
  ...
  fun customInstance_delegatesCorrectly() = runBlocking {
    ...
    val toolWindowWithPayloadsResult =
      instance.submitQueryWithPayloadsInToolWindow(
        project = project,
        query = "test tool window with payloads",
        blobs = blobs,
        payloads = payloads,
        conversationTarget = LlmConversationTarget.NewConversation,
      )

    assertThat(toolWindowWithPayloadsResult).isInstanceOf(LlmChatInToolWindowResult.Success::class.java)
    val successToolWithPayloads = toolWindowWithPayloadsResult as LlmChatInToolWindowResult.Success
    assertThat(successToolWithPayloads.conversationId).isEqualTo("conv-id")

    assertThat(fake.toolWindowQuery).isEqualTo("test tool window with payloads")
    assertThat(fake.editorBlobs).isEqualTo(blobs)
    assertThat(fake.editorPayloads).isEqualTo(payloads)
    assertThat(fake.conversationTarget).isEqualTo(LlmConversationTarget.NewConversation)

    val fileReferences = listOf<java.nio.file.Path>(java.nio.file.Paths.get("/src/MyClass.kt"))
    ...
    assertThat(fake.editorBlobs).isEqualTo(blobs)
    assertThat(fake.toolWindowFileReferences).isEqualTo(fileReferences)
    assertThat(fake.conversationTarget).isEqualTo(LlmConversationTarget.NewConversation)
  }
