...
class WhatsNewBundleCreatorTest {
  ...
  fun setUp() {
    ...
    whenever(mockUrlProvider.getWebConfig(ArgumentMatchers.anyString())).thenReturn(URL("file:$serverFile"))

    val resourceFile = getTestDataPath().resolve("whatsnewassistant/defaultresource-3.3.0.xml")
    whenever(mockUrlProvider.getResourceFileAsStream(ArgumentMatchers.any(), ArgumentMatchers.anyString())).thenAnswer {
