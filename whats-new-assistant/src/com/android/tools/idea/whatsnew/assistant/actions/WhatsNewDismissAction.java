...
final class WhatsNewDismissAction implements AssistActionHandler {
  ...
  public void handleAction(@NotNull ActionData actionData, @NotNull Project project) {
    ToolWindow assistantToolWindow = ToolWindowManager.getInstance(project).getToolWindow(OpenAssistSidePanelAction.TOOL_WINDOW_TITLE);
    if (assistantToolWindow != null) {
      assistantToolWindow.hide();
    }
    WhatsNewMetricsTracker.getInstance().dismissed(project);
  }
}
