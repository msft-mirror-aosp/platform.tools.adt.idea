...
/** Default [Icon] that [ProposedFileTreeModel] marks non-directory [File]s with when a proposedFileToIcon mapping isn't specified. */
...
/**
 * A [TreeModel] representing the sub-tree of the file system relevant to a pre-determined set of proposed new [File]s relative to a given
 * root directory. The model keeps track of which of the non-directory [File]s already existed **when the model was created**. If a path to
 * a proposed file contains non-existing intermediate directories, these will also be considered new files.
 *
 * The model marks each relevant file and directory with an [Icon] for rendering (see [ProposedFileTreeCellRenderer]). By default, proposed
 * files are marked with either [DIR_ICON] for directories or [DEFAULT_ICON] for regular files. However, callers can specify the [Icon]
 * corresponding to each proposed [File] by passing a [Map] to the appropriate constructor.
 *
 * Directory contents preserve the iteration order of the [Set] or [Map] used to construct the model. For example, if the model is built
 * from the sorted map
 *
 * { File("root/sub1/file1"): icon1, File("root/sub2/file2"): icon2 }
 *
 * then *sub1* will appear before *sub2* in the model of *root*'s children, since it appeared first when iterating over the map entries.
 */
class ProposedFileTreeModel private constructor(private val rootNode: Node) : TreeModel {
  ...
  constructor(rootDir: File, proposedFiles: Set<File>) : this(Node.makeTree(rootDir, proposedFiles) { null })
  ...
  /**
   * A vertex in a [ProposedFileTreeModel]'s underlying tree structure. Each node corresponds either to a directory, in which case it will
   * also keep track of a list of nodes corresponding to that directory's children, or to a proposed normal [File], in which case it records
   * whether or not the proposed [File] is in conflict with an existing file.
   *
   * By definition,[Node]s corresponding to directories are not conflicted (though they may represent a [conflictedTree]), and [Node]s
   * corresponding to normal (non-directory) files can have no children.
   *
   ...
   * @property children If this node corresponds to a directory, a list of nodes corresponding to the directory's children. Otherwise, this
   *   list is empty.
   * @property conflictedTree true if this node or any of its descendants correspond to a proposed [File] that already exists as a normal
   *   (non-directory) file.
   */
  data class Node(
    val file: File,
    ...
    /**
     * Given a node-relative path to a proposed [File] and an [Icon] with which to mark it, this function recursively builds all the missing
     * intermediate directory nodes between this node and a newly-constructed leaf node corresponding to the proposed [File].
     *
     * @param relativePath a list of path segments pointing to the proposed [File]
     * @param icon the [Icon] with which the proposed [File] should be marked, or null if the proposed [File] should be marked with a
     *   default [Icon].
     */
    ...
    companion object {
      /**
       * Constructs a tree rooted at [rootDir] that contains nodes corresponding to each proposed file in [proposedFiles] and any
       ...
       * @param getIconForFile a function which, given a file, returns either the icon that it should be marked with or null to use a
       *   default icon
       * @throws IllegalArgumentException if any of the files in [proposedFiles] is not a descendant of [rootDir]
       */
  ...
  private class ConflictChecker(private val rootDir: Path) {
    ...
    private fun Path.getChildren(): List<TypedFile> {
      return directoryContentsCache.computeIfAbsent(this) {
        try {
          return@computeIfAbsent Files.list(this).use { stream -> stream.map { TypedFile(it, Files.isDirectory(it)) }.toList() }
    ...
    private fun filesEqual(file1: Path?, file2: Path?): Boolean = pathsEqual(file1?.toString(), file2?.toString())
