...
class SkinDefinition private constructor(val layout: SkinLayout) {
  ...
  fun getRotatedFrameSize(displayRotationQuadrants: Int, displaySize: Dimension = layout.displaySize): Dimension {
    ...
    val size =
      if (scaleX == 1.0 && scaleY == 1.0) {
        ...
        Dimension(
          -frameRect.x.scaled(scaleX) + displaySize.width + (frameRect.right - layout.displaySize.width).scaled(scaleX),
  ...
  companion object {
    ...
    /** Crops the background image and breaks it into 8 pieces, 4 for sides and 4 for corners of the frame. internal */
    @JvmStatic
    ...
    /**
     * Breaks the background image into 8 pieces, 4 for sides and 4 for corners of the frame. Each piece is cropped to remove the fully
     * transparent part.
     */
    @JvmStatic
    ...
    @JvmStatic private fun BufferedImage.cropped(cropBounds: Rectangle): BufferedImage = getCroppedImage(this, cropBounds, -1)
