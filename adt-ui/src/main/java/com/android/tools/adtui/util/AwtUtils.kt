...
fun Dimension.scaled(scale: Double): Dimension = if (scale == 1.0) this else Dimension(width.scaled(scale), height.scaled(scale))
...
fun Point.scaled(scale: Double): Point = if (scale == 1.0) this else Point(x.scaled(scale), y.scaled(scale))

/** Returns this [Dimension] rotated by [numQuadrants] quadrants. */
fun Dimension.rotatedByQuadrants(numQuadrants: Int): Dimension = if (numQuadrants % 2 == 0) this else Dimension(height, width)
