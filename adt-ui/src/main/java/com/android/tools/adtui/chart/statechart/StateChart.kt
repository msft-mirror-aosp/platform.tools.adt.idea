...
class StateChart<T : Any>(
  private val model: StateChartModel<T>,
  ...
  constructor(
    ...
    model,
    colorProvider =
      object : StateChartColorProvider<T>() {
        override fun getColor(isMouseOver: Boolean, value: T): Color =
          colors[value]!!.let { if (isMouseOver) ColorUtil.brighter(it, 2) else it }
      },
  )
  ...
  private fun transformToViewSpace() {
    ...
    rectangleCache = series.mapIndexed { seriesIndex, data ->
      ...
      fun addRectangleDelta(value: T, previousX: Double, currentX: Double) {
        ...
        rectangles.add(
          Rectangle2D.Float(
            ((previousX - min) * invRange).toFloat(),
            barY.toFloat(),
            ((currentX - previousX) * invRange).toFloat(),
            barHeight,
          )
        )
        ...
        for ((x, value) in seriesDataList.subList(1, seriesDataList.size)) {
          if (value != previousValue) { // Ignore repeated values.
            ...
            if (x >= min) {
              ...
              previousValue?.let { addRectangleDelta(it, previousX, x.toDouble()) }
            }
  ...
  override fun draw(g2d: Graphics2D, dim: Dimension) {
    ...
    rectangleCache.forEachIndexed { seriesIndex, (rectangles, rectangleValues) ->
      fun List<Rectangle2D.Float>.searchByX(x: Float, w: Float) = binarySearch {
        when {
          it.x + it.width < x -> -1
          it.x > x + w -> 1
          else -> 0
      ...
      val endIndexExclusive =
        when {
          clipRect != null && clipRect.width != width ->
            rectangles.searchByX((clipRect.x + clipRect.width) / scaleX, 0f).let {
    ...
    return seriesIndexAtPoint(point)?.let { seriesIndex ->
      ...
      when {
        seriesData.isEmpty() -> null
          ...
          seriesIndex to
            when (val i = seriesData.binarySearch { it.x.compareTo(modelMouseX) }) {
  ...
  private fun seriesIndexAtPoint(point: Point) =
    (1f - point.y / height.toFloat()).let { normalizedY ->
  ...
  private fun renderUnion(container: Any?, containerOffset: Point) {
    if (rowPoint != null && container is Component) {
      getMouseRectanglesUnion(rowPoint!!)?.let { union ->
        ...
        // actual repaint.
        container.repaint(
          union.x.toInt() + containerOffset.x,
  ...
  private fun getMouseRectanglesUnion(mousePoint: Point) =
    seriesIndexAtMouse(mousePoint)?.let { (seriesIndex, i) ->
