 ...
 *
 * Unlike [GridBagLayout], which requires setting complex constraints and hard to reason about weights, [TabularLayout] works by setting
 * column definitions up front. A column can be fixed, fit-to-size, or proportional. The fit-to-size calculation uses a component's minimum
 * size, not preferred size, which may be useful to keep in mind.
 *
 ...
 * specified), but they can be configured for sizing as well. To set optional row definitions, use the [setRowSizing] method.
 *
 * When you register components with a panel using this layout, you must associate it with a [TabularLayout.Constraint] telling it which row
 * and column it should fit within. You should usually associate one element per cell, and (unless that cell is sized to fit) the element
 * will be stretched to fully contain the cell. You can additionally specify an element that spans across multiple columns, but be aware
 * that such elements are skipped when calculating the layout. It's also worth noting that invisible components are skipped over when
...
class TabularLayout(colSizes: Array<out SizingRule>, initialRowSizes: Array<out SizingRule>) : LayoutManager2 {
  ...
  data class SizingRule(
    ...
    enum class Type {
       ...
       *
       * For proportional columns, [value] is an integer value which should be compared with all other proportional columns to determine how
       * much space it gets. For example, if column A is set to 1 and column B is set to 3, column A gets 25% of all remaining space and
       * column B gets 75%.
       */
      PROPORTIONAL,
    }
  ...
  constructor(val row: Int, val col: Int, val rowSpan: Int, val colSpan: Int) {
    ...
    /**
     * Create a constraint which can live across multiple columns. Note that components which span across multiple columns aren't included
     * in fit-to-width layout calculations.
     */
  ...
  /**
   * Like constructor(String) but also specifying initial sizing conditions for rows. Unlike cols, row sizing can be added dynamically later
   * (using [setRowSizing]), but for the common case where the whole table size is known at creation time, this constructor allows it to be
   * expressed in a much more succinct manner.
   */
  constructor(
  ...
  override fun layoutContainer(parent: Container) {
    ...
    val visibleComponents =
      generateSequence(0) { it + 1 }.take(parent.componentCount).map { parent.getComponent(it) }.filter { it.isVisible }
  ...
  /**
   * Creates a calculator given an initial set of sizing rules. Use [notifySize] to fill it out with real values, and then call [getBounds]
   * to pull out the final sizing values.
   */
  constructor(private val rules: List<SizingRule>, private val gap: Int) {
     ...
     * @param numRules The number of rules that we should create. If [sparseRules] happens to have an index even greater than that, then
     *   `numRules` will be updated to contain it.
     */
    ...
    /**
     * Gets the dimensions for a component based on the type for the specified rule. The default sizing rule is minimum. The "Fit" sizing
     * rule uses preferred and is the only exception to this.
     */
     ...
     *
     * @param includeExtraSize Include the size needed to make space for the proportional cells as well. Minimum size calculations should
     *   ignore it while preferred size calculations should include it.
     */
    ...
    /**
     * Get a list of (pos, size) pairs, useful for setting the bounds of Swing components directly. For example, for columns with no gaps,
   ...
   * should be, and makes that data available through [rowCalculator] and [colCalculator] fields.
   */
