 ...
 *
 * Unlike [GridBagLayout], which requires setting complex constraints and hard to reason about
 * weights, [TabularLayout] works by setting column definitions up front. A column can be fixed,
 * fit-to-size, or proportional. The fit-to-size calculation uses a component's minimum size, not
 * preferred size, which may be useful to keep in mind.
 *
 * When a layout is requested, fixed and fit-to-width columns are calculated first, and all
 * remaining space is split by the proportional columns. Rows, in contrast, can be added on the fly.
 * By default, they are always fit-to-height (although a vertical gap can be specified), but they
 * can be configured for sizing as well. To set optional row definitions, use the [setRowSizing]
 * method.
 *
 * When you register components with a panel using this layout, you must associate it with a
 * [TabularLayout.Constraint] telling it which row and column it should fit within. You should
 * usually associate one element per cell, and (unless that cell is sized to fit) the element will
 * be stretched to fully contain the cell. You can additionally specify an element that spans across
 * multiple columns, but be aware that such elements are skipped when calculating the layout. It's
 * also worth noting that invisible components are skipped over when performing the layout.
 *
 * Columns are pre-allocated, so it is an error to specify a cell whose column index is out of
 * bounds. However, rows are unbounded - you can add a component at row 0 and then another at
 * row 1000. Still, if a row doesn't have any components inside of it, it will simply be skipped
 * during layout (i.e. sparse layouts are collapsed).
 *
 ...
 */
@UiThread
class TabularLayout(colSizes: Array<out SizingRule>, initialRowSizes: Array<out SizingRule>) :
  LayoutManager2 {
  ...
  /**
   * A definition for how to size a single column or row, indicating how its width or height will be
   * calculated during a layout.
   */
  data class SizingRule(
    ...
    enum class Type {
       ...
       *
       * For proportional columns, [value] is an integer value which should be compared with all
       * other proportional columns to determine how much space it gets. For example, if column A is
       * set to 1 and column B is set to 3, column A gets 25% of all remaining space and column B
       * gets 75%.
       */
      PROPORTIONAL,
    }
    ...
    enum class FitSizing {
       ...
       *
       * This is represented by a '-' at the end of "Fit". This is the default value for
       * uninitialized rows/columns. TODO (b/77491599) Update unassigned rows to use preferred in
       * place of minimum
       */
      MINIMUM,
      /**
    ...
    companion object {
      /**
       * Create a [SizingRule] from a string value, where each value represents either a Fit, Fixed,
       * or Proportional column.
       *
       * A Fit cell is represented by the string "Fit" (or "Fit-" for using min fit sizing). A Fixed
       * cell is represented by an integer + "px" (e.g. "100px"). A Proportional cell is represented
       * by an (optional) integer + "*" (e.g. "3*", "*").
       */
      fun fromString(s: String): SizingRule {
        fun String.getFitSizing() =
          FitSizing.MINIMUM.takeIf { endsWith('-') } ?: FitSizing.PREFERRED
  ...
  constructor(val row: Int, val col: Int, val rowSpan: Int, val colSpan: Int) {
     ...
     */
    @JvmOverloads constructor(row: Int, col: Int, colSpan: Int = 1) : this(row, col, 1, colSpan)
  ...
  /**
   * Create a [TabularLayout] from a comma-delimited string of values that are valid for creating a
   * [SizingRule].
   *
   * Examples:
  ...
  /**
   * Like constructor(String) but also specifying initial sizing conditions for rows. Unlike cols,
   * row sizing can be added dynamically later (using [setRowSizing]), but for the common case where
   * the whole table size is known at creation time, this constructor allows it to be expressed in a
   * much more succinct manner.
   */
  constructor(
    colSizesString: String,
    initialRowSizesString: String,
  ...
  /** @see SizingRule.fromString */
  fun setRowSizing(rowIndex: Int, rowSizeString: String): TabularLayout =
    setRowSizing(rowIndex, SizingRule.fromString(rowSizeString))

  override fun addLayoutComponent(comp: Component, constraint: Any) {
    require(constraint is Constraint) {
      "Children of containers using ${javaClass.simpleName} must be added with a constraint"
    }
  ...
  /**
   * Creates a calculator given an initial set of sizing rules. Use [notifySize] to fill it out with
   * real values, and then call [getBounds] to pull out the final sizing values.
   */
  constructor(private val rules: List<SizingRule>, private val gap: Int) {
     ...
     * useful for rows, where rows can be created on the fly and are set to a default sizing rule in
     * that case.
     *
     * @param sparseRules A mapping of indices to rules. Any missing index will be assumed to be
     *   fit-to-size.
     * @param numRules The number of rules that we should create. If [sparseRules] happens to have
     *   an index even greater than that, then `numRules` will be updated to contain it.
     */
    constructor(
      sparseRules: Map<Int, SizingRule>,
      numRules: Int,
      gap: Int,
    ) : this(fromSparseRules(sparseRules, numRules), gap)

    init {
      val totalProportionalSize =
        rules.filter { it.type == SizingRule.Type.PROPORTIONAL }.sumOf { it.value }.toFloat()
    ...
    /**
     * Gets the dimensions for a component based on the type for the specified rule. The default
     * sizing rule is minimum. The "Fit" sizing rule uses preferred and is the only exception to
     * this.
     */
    fun getComponentDimension(i: Int, c: Component): Dimension =
      if (rules[i].value == SizingRule.FitSizing.PREFERRED.ordinal) c.preferredSize
      else c.minimumSize
     ...
     *   well. Minimum size calculations should ignore it while preferred size calculations should
     *   include it.
     */
  ...
  private data class PosSize(var pos: Int = 0, var size: Int = 0)

  /**
   * Class which, when instantiated on a parent container, runs through all its components,
   * calculates what the sizes of rows and columns should be, and makes that data available through
   * [rowCalculator] and [colCalculator] fields.
   */
...
private fun fromSparseRules(
  sparseRules: Map<Int, TabularLayout.SizingRule>,
  rulesCount: Int,
): List<TabularLayout.SizingRule> {
  ...
  return (0 until rulesCount + additionalRulesCount).map {
    ...
    sparseRules[it]
      ?: TabularLayout.SizingRule(
        TabularLayout.SizingRule.Type.FIT,
        TabularLayout.SizingRule.FitSizing.MINIMUM.ordinal,
      )
  }
}
