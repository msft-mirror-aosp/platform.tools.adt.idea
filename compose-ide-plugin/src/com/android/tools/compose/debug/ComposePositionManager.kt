 ...
 *
 * This class essentially resolves breakpoints for lambdas generated by the compose compiler optimization that was introduced in
 * I8c967b14c5d9bf67e5646e60f630f2e29e006366 The default [KotlinPositionManager] only locates source positions in enclosing and nested
...
class ComposePositionManager(private val debugProcess: DebugProcess, private val kotlinPositionManager: KotlinPositionManager) :
   ...
   *
   * This handles the case where a user sets a breakpoint in a ComposableSingleton lambda after the debug process has already initialized
   * the corresponding `lambda-n` class.
   */
  ...
  /**
   * Registers search patterns in the form of [ClassPrepareRequest]s for classes which may contain the given `position`, but may not be
   * loaded yet. The `requestor` will be called for any newly prepared class which matches any of the created search patterns.
   */
  override fun createPrepareRequests(requestor: ClassPrepareRequestor, position: SourcePosition): List<ClassPrepareRequest> {
    ...
    val singletonRequest =
      debugProcess.requestsManager.createClassPrepareRequest(requestor, "${computeComposableSingletonsClassName(file)}\$*")

    return if (singletonRequest == null) kotlinRequests else kotlinRequests + singletonRequest
  }

  /**
   * A method from [PositionManager] which was superseded by [createPrepareRequests] in [MultiRequestPositionManager]. Intellij code should
   * never call this method for subclasses of [MultiRequestPositionManager].
   */
