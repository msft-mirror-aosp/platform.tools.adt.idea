...
class UpdateReferenceImagesDialog(
  ...
  private enum class ScreenshotViewType(val displayText: String) {
  ...
  private fun createPreviewToolbar(): ComposePanel {
    return ComposePanel().apply {
      setContent {
        SwingBridgeTheme {
          var selectedTab by remember { mutableStateOf(ScreenshotViewType.NEW) }
          val buttonData = remember(selectedTab) {
            ScreenshotViewType.values().map { viewId ->
              SegmentedControlButtonData(
                selected = viewId == selectedTab,
                content = { _ -> Text(viewId.displayText) },
                onSelect = { selectedTab = viewId },
              )
            }
          }
          Row(
