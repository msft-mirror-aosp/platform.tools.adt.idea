...
internal val <T : Any> Annotated<PropertyValue<T>>.resolved
  get() = value.resolved
internal val <T : Any> Annotated<PropertyValue<T>>.parsedValue
  get() = value.parsedValue

fun PsModelDescriptor.testEnumerateProperties(): Set<ModelProperty<*, *, *, *>> {
  val result = mutableSetOf<ModelProperty<*, *, *, *>>()
  enumerateProperties(
    receiver =
      object : PsModelDescriptor.PropertyReceiver {
    ...
    .filter {
      it.parameters.size == 1 &&
        ...
        it.returnType.isSubtypeOf(
          ModelProperty::class.createType(listOf(KTypeProjection.STAR, KTypeProjection.STAR, KTypeProjection.STAR, KTypeProjection.STAR))
        )
    }
