...
class ParsedValueRendererTest {
  ...
  fun testBuildKnownValueRenderers_withVariables() {
    ...
    val valueDescriptors =
      listOf(ValueDescriptor(true), ValueDescriptor(false), ValueDescriptor(notDescribedTrue), ValueDescriptor(describedFalse, "Lie"))
    val renderers = buildKnownValueRenderers(knownValuesFrom(valueDescriptors), Any::testToString, false)
  ...
  fun testRenderParsedValue_notSet_annotated() {
    assertThat(
      ParsedValue.NotSet.annotateWithError("error")
        .testRenderWith<Int>(buildKnownValueRenderers(emptyKnownValues(), Any::testToString, null)),
      equalTo("<error>(error)"),
    )
    assertThat(
      ParsedValue.NotSet.annotateWithError("error")
    ...
    assertThat(
      ParsedValue.NotSet.annotateWithError("error")
        .testRenderWith<Int>(buildKnownValueRenderers(knownValuesFrom(listOf(ValueDescriptor(1, "One"))), Any::testToString, 1)),
  ...
  fun testRenderParsedValue_setValue_annotated() {
    ...
    assertThat(
      one
        .annotateWithError("error")
        .testRenderWith<Int>(buildKnownValueRenderers(knownValuesFrom(listOf(ValueDescriptor(1, "One"))), Any::testToString, null)),
  ...
  fun testRenderParsedValue_setReference() {
    ...
    assertThat(
      var1.testRenderWith<Int>(buildKnownValueRenderers(emptyKnownValues(), Any::testToString, null)),
  ...
  fun testRenderParsedValue_setInterpolatedString() {
    ...
    assertThat(
      interpolated.testRenderWith<String>(buildKnownValueRenderers(emptyKnownValues(), Any::testToString, null)),
      equalTo("<var>\"\$var1 and \$var2\"<comment> : \"a nd b@\""),
    )
    assertThat(
      interpolated.testRenderWith<String>(
  ...
  fun testRenderParsedValue_setOtherDsl() {
    ...
    assertThat(
      dsl.testRenderWith<String>(buildKnownValueRenderers(emptyKnownValues(), Any::testToString, null)),
  ...
  fun testRenderParsedValue_setInvalid() {
    ...
    assertThat(
      error.testRenderWith<String>(buildKnownValueRenderers(emptyKnownValues(), Any::testToString, null)),
  ...
  fun testRenderAny_list() {
    ...
    assertThat(testRender { list.renderAnyTo(this, mapOf()) }, equalTo("[1, a, <var>\$v<comment> : true<nocolor>]"))
  }

  @Test
  fun testRenderAny_map() {
    val map = mapOf("a" to 1.asParsed(), "b" to "a".asParsed(), "c" to ("v" to true).asParsed()).asParsed()
    assertThat(testRender { map.renderAnyTo(this, mapOf()) }, equalTo("[a : 1, b : a, c : <var>\$v<comment> : true<nocolor>]"))
  }

  @Test
  fun testRenderAny_mapWithRferencesToNestedMaps() {
    val map =
      mapOf("a" to (mapOf("b" to "val".asParsed())).asParsed(), "c" to (mapOf("d" to ("var" to "val").asParsed())).asParsed()).asParsed()
    assertThat(testRender { map.renderAnyTo(this, mapOf()) }, equalTo("[a : [b : val], c : [d : <var>\$var<comment> : val<nocolor>]]"))
  }

  @Test
  fun testRenderAny_referenceToMapWithRferencesToNestedMaps() {
    val map =
      ("ref" to mapOf("a" to (mapOf("b" to "val".asParsed())).asParsed(), "c" to (mapOf("d" to ("var" to "val").asParsed())).asParsed()))
        .asParsed()
    assertThat(testRender { map.renderAnyTo(this, mapOf()) }, equalTo("<var>\$ref<comment> : [a : [b : val], c : [d : \$var]]"))
  }
}
...
private fun <T : Any> knownValuesFrom(literals: List<ValueDescriptor<T>>) =
