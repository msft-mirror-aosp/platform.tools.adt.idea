...
class PsModuleCollection(parent: PsProjectImpl) : PsMutableCollectionBase<PsModule, ModuleKey, PsProjectImpl>(parent) {
  ...
  override fun create(key: ModuleKey): PsModule =
    ...
    }.also { module -> onModuleChangedHandlers.forEach { (disposable, handler) -> module.onChange(disposable, handler) } }
  ...
  override fun update(key: ModuleKey, model: PsModule) {
    ...
    val moduleName =
      if (gradlePathName.isNotEmpty()) gradlePathName else parent.ideProject.getHolderModuleByGradlePath(key.gradlePath)?.name ?: "Unknown"

    val moduleResolvedModel = parent.getResolvedModuleModelsByGradlePath()[key.gradlePath]
  ...
  override fun removeExisting(key: ModuleKey) {
    ...
    for (module in parent.modules) {
      module.dependencies.let { dependencies ->
        dependencies.findModuleDependencies(key.gradlePath).forEach { module.removeDependency(it) }
      }
    }
  ...
  private fun findParentModuleFor(gradlePath: String): PsModule? {
    ...
    while (remainingPath.contains(SdkConstants.GRADLE_PATH_SEPARATOR)) {
      ...
      entries.entries
        .find { it.key.gradlePath == remainingPath }
        ?.let {
          return it.value
        }
    }
...
fun Project.getHolderModuleByGradlePath(gradlePath: String) =
  ...
  ModuleManager.getInstance(this) // Use ideProject to find the name of the module.
    .modules
