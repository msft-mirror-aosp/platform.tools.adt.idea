...
fun androidSdkSuggestions(): AndroidSdkSuggestions {
  ...
  return if (sdkHandler != null) {
    ...
    androidSdkSuggestions(
      sdkHandler.getRepoManagerAndLoadSynchronously(logger),
      sdkHandler.getAndroidTargetManager(logger).getTargets(logger),
      knownStableAndroidVersions(),
    )
  } else {
    AndroidSdkSuggestions(
      minSdks = listOf(),
...
@VisibleForTesting
fun androidSdkSuggestions(
  sdkManager: RepoManager,
  targets: Collection<IAndroidTarget>,
  knownAndroidVersions: Set<AndroidVersion>,
): AndroidSdkSuggestions {
  fun platformName(target: IAndroidTarget) =
    if (target.isPlatform)
      if (target.version == AndroidVersion(36, 0)) "36" // TODO(b/411099168) Can this case be simplified?
      else target.version.apiStringWithExtension.takeIf { it.toIntOrNull() != null } ?: target.version.platformHashString
    else AndroidTargetHash.getAddonHashString(target.vendor, target.name, target.version)
  ...
  return AndroidSdkSuggestions(
    ...
    targetSdks =
      majorVersions.map {
        ValueDescriptor(it.minOrTargetPlatformName(), it.fullApiName())
      }, // TODO(b/414551906): Raise the lower limit of what is suggested here?
