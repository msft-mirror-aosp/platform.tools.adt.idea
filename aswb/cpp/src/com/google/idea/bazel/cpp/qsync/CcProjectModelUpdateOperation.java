 ...
 */
package com.google.idea.bazel.cpp.qsync;
...
import com.google.idea.bazel.cpp.CppSupportChecker;
...
public class CcProjectModelUpdateOperation implements Disposable {
  ...
  private void visitLanguageCompilerSettingsMap(
    ...
    for (Map.Entry<CcLanguage, CcCompilerSettings> e : map.entrySet()) {
      ...
      CLanguageKind lang = getLanguageKind(e.getKey(), "compiler settings");
  ...
  private void visitSourceFile(
      CcCompilationContext compilationContext,
      CcSourceFile source,
      OCResolveConfiguration.ModifiableModel config) {
    CcCompilerSettings compilerSettings =
        compilationContext.getLanguageToCompilerSettings().get(source.getLanguage());
    if (compilerSettings == null) {
      // If we get a new file without fetching configs we may end up with a new language if
      // populated lazily.  This will practically never
      // happen as there are just C and CPP and in the presence of both toolchains both are always
      // populated.
      logger.error(
          String.format(
              "No compiler setting for %s in context %s",
              source.getLanguage(), compilationContext.getId()));
      return;
    }
    CidrCompilerSwitches switches =
    ...
    perSourceCompilerSettings.setCompiler(
        ClangCompilerKind.INSTANCE, getCompilerExecutable(compilerSettings), compilerWorkingDir);
  }
  ...
  private File getCompilerExecutable(CcCompilerSettings compilerSettings) {
    return pathResolver.resolve(compilerSettings.getCompilerExecutablePath()).toFile();
  }
