...
def _package_dependencies_impl(target, ctx):
    ...
    return [OutputGroupInfo(
        ...
        qs_external_transitive_runtime_jars = dep_info.external_transitive_runtime_jars,
        qs_info = java_info_files,
        qs_jdeps = _noneToEmpty(dep_info.compile_jdeps),
        qs_aars = _noneToEmpty(dep_info.aars),
        qs_gensrcs = depset(
            transitive = [
                _noneToEmpty(dep_info.gensrcs),
                _noneToEmpty(dep_info.gen_android_res),
            ],
        ),
...
DependenciesInfo = provider(
    ...
    fields = {
        ...
        "external_transitive_runtime_jars": "a depset of the external transitive runtime JARs",
...
def create_dependencies_info(
        ...
        external_transitive_runtime_jars = depset(),
    ...
    return DependenciesInfo(
        ...
        external_transitive_runtime_jars = external_transitive_runtime_jars,
...
def create_java_dependencies_info(
        ...
        info_files,
        compile_time_jars,
        external_transitive_runtime_jars,
        compile_jdeps,
        aars,
        gensrcs,
        gen_android_res,
...
def merge_dependencies_info(
    ...
    merged = create_dependencies_info(
        ...
        external_transitive_runtime_jars = java_dep_info.external_transitive_runtime_jars if java_dep_info else depset(),
...
def _encode_target_info_proto(target_info):
    contents = struct(
        ...
        gen_android_res = _encode_file_list(target_info.gen_android_res),
...
def _collect_own_java_artifacts(
    ...
    if must_build_main_artifacts:
        ...
        if android_info != None:
            ...
            own_gen_android_res_files += android_info.generated_resource_files
    ...
    return struct(
        ...
        gen_android_res_depset = depset(own_gen_android_res_files),
...
def _collect_own_and_dependency_java_artifacts(
    ...
    own_and_transitive_gen_android_res_depsets = []
    ...
    for info in dependency_infos.values():
        ...
        own_and_transitive_gen_android_res_depsets.append(info.gen_android_res)
    ...
    return struct(
        ...
        gen_android_res = depset(own_files.gen_android_res_depset.to_list(), transitive = own_and_transitive_gen_android_res_depsets),
        external_transitive_runtime_jars = _get_external_transitive_runtime_jars(
            not target_is_within_project_scope,
...
def _get_external_transitive_runtime_jars(is_external, transitive_runtime_jars, dependency_infos):
    return transitive_runtime_jars if is_external else depset(
        transitive = [dep.external_transitive_runtime_jars for dep in dependency_infos.values()],
    )
...
def _collect_java_dependencies_core_impl(
    ...
    gen_android_res = own_and_dependencies.gen_android_res
    ...
    return create_java_dependencies_info(
        ...
        external_transitive_runtime_jars = own_and_dependencies.external_transitive_runtime_jars,
        compile_jdeps = compile_jdeps,
        aars = aars,
        gensrcs = gensrcs,
        gen_android_res = gen_android_res,
