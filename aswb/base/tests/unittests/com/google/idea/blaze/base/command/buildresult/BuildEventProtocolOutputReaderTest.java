...
public class BuildEventProtocolOutputReaderTest extends BlazeTestCase {
  ...
  public void parseAllOutputs_singleTargetEvents_returnsAllOutputs() throws Exception {
    ...
    List<OutputArtifact> parsedFilenames =
        BepParser.parseBepArtifacts(
                BuildEventStreamProvider.fromInputStream(asInputStream(events)), null)
            .getAllOutputArtifactsForTesting();

    assertThat(
            getLocalFiles(
                Label.of(label), parsedFilenames, context, getProject(), RuntimeArtifactKind.JAR))
  ...
  public void parseAllOutputs_nonFileEvent_returnsEmptyList() throws Exception {
    ...
    List<OutputArtifact> parsedFilenames =
        BepParser.parseBepArtifacts(
                BuildEventStreamProvider.fromInputStream(asInputStream(targetFinishedEvent)), null)
            .getAllOutputArtifactsForTesting();
  ...
  public void parseTestResults_singleEvent_returnsTestResults() throws Exception {
    ...
    BlazeTestResults results =
        BuildEventProtocolOutputReader.parseTestResults(
            BuildEventStreamProvider.fromInputStream(asInputStream(event)));
  ...
  public void parseTestResults_singleTestEventWithTargetConfigured_resultsIncludeTargetKind()
    ...
    BlazeTestResults results =
        BuildEventProtocolOutputReader.parseTestResults(
            BuildEventStreamProvider.fromInputStream(events));
  ...
  public void parseTestResults_singleTargetWithMultipleEvents_returnsTestResults()
    ...
    BlazeTestResults results =
        BuildEventProtocolOutputReader.parseTestResults(
            BuildEventStreamProvider.fromInputStream(asInputStream(events)));
  ...
  public void parseTestResults_multipleEvents_returnsAllResults() throws Exception {
    ...
    BlazeTestResults results =
        BuildEventProtocolOutputReader.parseTestResults(
            BuildEventStreamProvider.fromInputStream(asInputStream(test1, test2)));
  ...
  private static ImmutableList<File> getLocalFiles(
      Label target,
      List<? extends OutputArtifact> artifacts,
      BlazeContext context,
      Project project,
    ...
    return runtimeArtifactCache.fetchArtifacts(target, artifacts, context, artifactKind).stream()
        .map(Path::toFile)
        .collect(toImmutableList());
  }

  private static ImmutableList<File> getOutputXmlFiles(
      Label label, BlazeTestResult result, BlazeContext context, Project project) {
    return getLocalFiles(
        label,
        new ArrayList<>(result.getOutputXmlFiles()),
        context,
        project,
