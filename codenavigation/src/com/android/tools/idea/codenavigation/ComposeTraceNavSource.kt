 ...
 *
 * @param mavenSignatureResolver returns library signature { groupId, artifactId, version } for a given PsiFile or `null` if it cannot
...
internal constructor(
  ...
  constructor(
    ...
    createNavigatable = { files, lineNumber ->
      if (files.isEmpty()) null else MultiNavigatable(files.map { OpenFileDescriptor(project, it.virtualFile, lineNumber, -1) })
    },
  ...
  override fun lookUp(location: CodeLocation, arch: String?): Navigatable? {
    ...
    try {
      val result =
        getFilesByName(fileName) // Find files matching required file name
      ...
      logger.warn(
        "Issue while trying to navigate to location: fullComposableName=$fullComposableName, fileName=$fileName, " +
