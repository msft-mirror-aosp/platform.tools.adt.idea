...
class ProfilingConfigurationTest {
  ...
  fun fromProtoForPerfettoOptionsWithTraceboxDisabled() {
    val proto = TraceConfiguration.newBuilder().setPerfettoOptions(PerfettoConfig.TraceConfig.newBuilder().build()).build()
    val config = ProfilingConfiguration.fromProto(proto, false)
  ...
  fun addOptionsSimpleperfConfigAddsSuccessfully() {
    val configBuilder = TraceConfiguration.getDefaultInstance().toBuilder()
    val simpleperfConfiguration = SimpleperfConfiguration("MyConfiguration").apply { profilingSamplingIntervalUs = 1234 }
