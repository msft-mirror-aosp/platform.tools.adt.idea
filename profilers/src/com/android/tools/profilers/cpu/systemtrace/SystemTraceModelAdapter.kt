...
/**
 * SystemTraceModelAdapter exposes a common API for accessing the raw model data from system trace captures.
 *
...
interface SystemTraceModelAdapter {
  ...
  /**
   * @return a ThreadModel if we have information for a possible dangling thread with that thread id, which is a thread that we don't have
   *   the information about which process they belong to.
   */
  ...
  /**
   * @return true if there is potentially missing data from the capture. It's hard to guarantee if data is missing or not, so this is a best
   *   guess.
   */
...
data class ProcessModel(
  ...
  companion object {
    ...
    @JvmStatic val serialVersionUID = -7303268805551316288L
  }
  ...
  /**
   * Returns the best assumed name for a process. If the process does not have a name it looks at the name of the main thread, but if we
...
data class ThreadModel(
  ...
  val threadStateEvents: List<ThreadStateModel>,
) : Serializable {

  companion object {
    // generated by serialver
    @JvmStatic val serialVersionUID = -172242225241232844L
  }
}
...
data class TraceEventModel(
  ...
  companion object {
    // generated by serialver
    @JvmStatic val serialVersionUID = -2496458872098350643L
  }
}
...
data class SchedulingEventModel(
  ...
  companion object {
    // generated by serialver
    @JvmStatic val serialVersionUID = -2824580527083393668L
  }
}

data class CounterModel(val name: String, val valuesByTimestampUs: SortedMap<Long, Double>) : Serializable {

  companion object {
    // generated by serialver
    @JvmStatic val serialVersionUID = -305070684614208809L
  }
}

data class ThreadStateModel(val state: ThreadState, val startTimestampUs: Long, val endTimestampUs: Long) : Serializable {

  companion object {
    // generated by serialver
    @JvmStatic val serialVersionUID = 4037744707206656133L
  }
}
...
object CounterDataUtils {
  ...
  /**
   * This utility function takes in a list of counters and a map indicating the desired grouping. It will return a new map with grouped
   * counters' data aggregated.
   */
  @JvmStatic
  fun aggregateCounters(
    counters: List<CounterModel>,
    groupingMap: Map<String, String>,
    normalizeStartTime: Boolean,
  ): SortedMap<String, List<SeriesData<Long>>> {
    ...
    aggregatableCounters
      .filter { groupingMap.contains(it.key) }
      .forEach {
  ...
  /**
   * In order to perform the aggregation in 'aggregateCounters', the 'valuesByTimestampUs' field of the CounterModel must be of type
   * TreeMap<Long, Double>, rather than SortedMap<Long, Double>. This helper function does this type conversion to guarantee we have data in
   * aggregatable format.
   *
   * @param counters - list of CounterModel's to be transformed
   * @return a mapping from the counter name (String) to a map of the counter data (TreeMap). This TreeMap maps a timestamp (Long) to a
   *   corresponding value (Double), and is sorted by timestamp (the key) in increasing order.
   */
  private fun convertCountersToAggregatableFormat(counters: List<CounterModel>): Map<String, TreeMap<Long, Double>> {
    return counters.associate { it.name to TreeMap<Long, Double>(it.valuesByTimestampUs) }
  }

  /**
   * This function clips a grouped counter's series data to start at the maximum start time of the grouped counters. This is useful when
   * combining groups that have much different magnitudes, as it removes the first points of the combined series that have extreme
   * differences b/w them.
   */
  private fun removeDataBeforeNormalizedStartTime(
    groupToAggregatedData: MutableMap<String, TreeMap<Long, Double>>,
  ...
  /**
   * This utility function takes in counters data map (counter name to a list of series data). It will return a new map where the counter
   * data value at each timestamp is the difference between itself and the last timestamp's value.
   */
  @JvmStatic
...
data class CpuCoreModel(val id: Int, val schedulingEvents: List<SchedulingEventModel>, val countersMap: Map<String, CounterModel>) :
  Serializable {

  companion object {
    // generated by serialver
    @JvmStatic val serialVersionUID = 8233672032802842718L
  }
}

/**
 * @param appJankType Raw data may contain multiple jank types but we only extract the app jank type, namely "App Deadline Missed", "Buffer
 *   Stuffing" and "Unknown Jank". If no jank is present, the value is "None". If no app jank is present, the value is "Unspecified".
 */
data class AndroidFrameTimelineEvent(
  ...
  val expectedDurationUs
    get() = expectedEndUs - expectedStartUs

  val actualDurationUs
    get() = actualEndUs - expectedStartUs

  val isJank
    get() = appJankType != PerfettoTrace.FrameTimelineEvent.JankType.JANK_NONE

  val isActionableJank
    get() = appJankType == PerfettoTrace.FrameTimelineEvent.JankType.JANK_APP_DEADLINE_MISSED
}

/**
 * Given a list of events X with starts and ends, return a list of padded events SeriesData<Y>, where Y subsumes X and padding values.
 *
