...
/**
 * A service for tracking events that occur in our profilers, in order to understand and evaluate how our users are using them.
 *
 * The class that implements this should be sure to let users opt out of sending this information, at which point all these methods should
 * become no-ops.
 */
interface FeatureTracker {
  /** Track when the TransportDeviceManager detects an ddmlib device and is about to begin the logic to launch the transport daemon. */
  ...
  /** Track when we enter a new stage. The stage should always be included as state with all other tracking events. */
  fun trackEnterStage(stageType: AndroidProfilerEvent.Stage)

  /**
   * Track when the user clicks the Profile button. This can happen via the tool bar, or the run menu. This is in contrast to when the user
   * attaches profilers to an app that was already running.
   *
   * Can optionally set metadata like profiling mode, etc.
   */
  fun trackRunWithProfiling(metadata: RunWithProfilingMetadata)

  /** Track when auto profiling is requested. e.g. when the user clicks "Profile", or "Run"/"Debug" while the profilers window is opened. */
  ...
  /**
   * Track when we learn that the process we are profiling is instrumented and allows us to query advanced profiling information. This value
   * will always be a subset of [.trackProfilingStarted].
   */
  fun trackAdvancedProfilingStarted()

  /** Track when the user takes an action to change the current device. This will only be tracked if the device actually changes. */
  fun trackChangeDevice(device: Common.Device)

  /** Track when the user takes an action to change the current process. This will only be tracked if the process actually changes. */
  ...
  /** Track when the user takes an action to return back to the top-level monitor view (from a specific profiler). */
  fun trackGoBack()
  ...
  /**
   * Track an attempt of spawning a new instance of the Trace Processor Daemon and how long it took to either the instance is ready to serve
   * requests (if the spawn was successful) or how long we took to detect that we failed to spawn the new instance.
   */
   ...
   */
  fun trackTraceProcessorLoadTrace(queryStatus: QueryReturnStatus, methodTimeMs: Long, queryTimeMs: Long, traceSizeBytes: Long)
   ...
   * @param queryTimeMs how long - in milliseconds - we spent waiting for the query to return from TPD.
   */
  fun trackTraceProcessorProcessMetadata(queryStatus: QueryReturnStatus, methodTimeMs: Long, queryTimeMs: Long)
 ...
 * @param measure the size of the in-memory representation (e.g. object count, event count, etc.) that's only queried if the task succeeds
 */
fun <A> FeatureTracker.trackLoading(type: AndroidProfilerEvent.Loading.Type, sizeKb: Int, measure: () -> Long, run: () -> A): A {
  ...
  val studioFree = (CommonMetricsData.jvmDetails.maximumHeapSize - CommonMetricsData.javaProcessStats.heapMemoryUsage).bToMb()

  fun track(setUp: AndroidProfilerEvent.Loading.Builder.() -> Unit) =
    trackLoading(
      AndroidProfilerEvent.Loading.newBuilder()
        .setType(type)
        .setSizeKb(sizeKb)
